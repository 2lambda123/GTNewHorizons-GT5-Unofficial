...
public interface IMachineProgress
    extends IHasWorldObjectAndCoords, MachineProgress {

  /**
   * returns the Progress this Machine has made. Warning, this can also be
   * negative!
   */
  int getProgress();
   ...
   * Used for Translocators, which need to check if they need to transfer
   * immediately.
   */
  boolean hasWorkJustBeenEnabled();

  /**
   * used to control Machines via Redstone Signal Strength by special Covers In
   * case of 0 the Machine is very likely doing nothing, or is just not being
   * controlled at all.
   */
  ...
  /**
   * Indicates if the object in question was forced to shut down (i.e. loss of
   * power)
   */
  default boolean wasShutdown() { return false; }
}
