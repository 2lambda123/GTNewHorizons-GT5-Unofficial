...
public class GT_Packet_MultiTileEntity extends GT_Packet_New {
  ...
  public static final int COVERS = B[0], REDSTONE = B[1], MODES = B[2],
                          CONTROLLER = B[3], INVENTORY_INDEX = B[4],
                          INVENTORY_NAME_ID = B[5], BOOLEANS = B[6],
                          SOUND = B[7];

  public GT_Packet_MultiTileEntity(boolean reference) { super(reference); }

  @Override
  public void encode(ByteBuf aOut) {
    Set<PacketData<MultiTileEntityProcess>> set =
        data.stream().sorted().collect(
  ...
  public GT_Packet_New decode(ByteArrayDataInput in) {
    ...
    final GT_Packet_MultiTileEntity packet =
    ...
    Set<PacketData<MultiTileEntityProcess>> set =
        packet.data.stream().sorted().collect(
            HashSet<PacketData<MultiTileEntityProcess>>::new,
            HashSet<PacketData<MultiTileEntityProcess>>::add,
            HashSet<PacketData<MultiTileEntityProcess>>::addAll);
    packet.clearData();
  ...
  public void process(IBlockAccess aWorld) {
     ...
     * final IMultiTileEntity mte = mteBlock.receiveMultiTileEntityData(aWorld,
     * mX, mY, mZ, mRID, mID); if (mte == null) return;
     * mte.receiveClientData(GregTechTileClientEvents.CHANGE_COMMON_DATA,
     * mCommonData);
     ...
     * mte.receiveClientData(GregTechTileClientEvents.CHANGE_REDSTONE_OUTPUT,
     * mRedstone);
     * }
     * if ((features & MODES) == MODES && mte instanceof
     * IMultiTileEntity.IMTE_HasModes mteModes) { mteModes.setMode(mode);
     * mteModes.setAllowedModes(allowedModes);
     * }
     * if ((features & INVENTORY_NAME_ID) == INVENTORY_NAME_ID && mte instanceof
     * Inventory invUpg) { invUpg.setInventoryName(mInventoryName);
     * invUpg.setInventoryId(inventoryID);
     * }
     * if ((features & CONTROLLER) == CONTROLLER && mte instanceof
     * IMultiBlockPart) { final IMultiBlockPart mtePart = (IMultiBlockPart) mte;
     * mtePart.setTargetPos(mTargetPos);
     * }
     * if ((features & INVENTORY_INDEX) == INVENTORY_INDEX && mte instanceof
     * IMultiBlockPart) { final IMultiBlockPart mtePart = (IMultiBlockPart) mte;
     * mtePart.setLockedInventoryIndex(mLockedInventoryIndex);
     * }
     * if ((features & BOOLEANS) == BOOLEANS && mte instanceof
     * IMultiTileMachine) { final IMultiTileMachine machine =
     * (IMultiTileMachine) mte; machine.setBooleans(booleans);
     * }
