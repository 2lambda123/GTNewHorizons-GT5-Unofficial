...
public class MultiTileEntityRegistry {
  ...
  private static final HashMap<String, MultiTileEntityRegistry>
      NAMED_REGISTRIES = new HashMap<>();

  // TODO: NBT sensitive or not? Starting with not for now
  private static final ItemStackMap<MultiTileEntityRegistry> REGISTRIES =
      new ItemStackMap<>(false);
  private static final HashSet<Class<?>> registeredTileEntities =
      new HashSet<>();
  ...
  private static MultiTileEntityBlock
  regblock(String internalName, MultiTileEntityBlock block,
           Class<? extends ItemBlock> itemClass) {
    GameRegistry.registerBlock(
        block, itemClass == null ? ItemBlock.class : itemClass, internalName);
    return block;
  }
  ...
  public MultiTileEntityRegistry(String internalName,
    ...
    this.block = block;
    GT_FML_LOGGER.info(internalName + " " + Block.getIdFromBlock(block) +
                       "This is the answer");
    this.block.setRegistry(this);
    REGISTRIES.put(new ItemStack(Item.getItemById(Block.getIdFromBlock(block)),
                                 1, GT_Values.W),
                   this);
  ...
  public static MultiTileEntityRegistry getRegistry(int aRegistryID) {
    return REGISTRIES.get(
        new ItemStack(Item.getItemById(aRegistryID), 1, GT_Values.W));
  }
  ...
  create(int metaId, Class<? extends MultiTileEntity> clazz) {
    return new MultiTileEntityClassContainer(this, block, metaId, clazz);
  }

  /**
   * Adds a new MultiTileEntity. It is highly recommended to do this in either
   * the PreInit or the Init Phase. PostInit might not work well.
   */
  public ItemStack add(String categoryName,
                       MultiTileEntityClassContainer classContainer) {
    boolean tFailed = false;
    if (classContainer == null) {
      GT_FML_LOGGER.error(
          "MULTI-TILE REGISTRY ERROR: Class Container is null!");
      tFailed = true;
    } else {
      if (classContainer.getClazz() == null) {
        GT_FML_LOGGER.error(
            "MULTI-TILE REGISTRY ERROR: Class inside Class Container is null!");
        tFailed = true;
      }
      if (classContainer.getMetaId() == GT_Values.W) {
        GT_FML_LOGGER.error(
            "MULTI-TILE REGISTRY ERROR: Class Container uses Wildcard MetaData!");
        tFailed = true;
      }
      if (classContainer.getMetaId() < 0) {
        GT_FML_LOGGER.error(
            "MULTI-TILE REGISTRY ERROR: Class Container uses negative MetaData!");
        tFailed = true;
      }
      if (registry.containsKey(classContainer.getMetaId())) {
        GT_FML_LOGGER.error(
            "MULTI-TILE REGISTRY ERROR: Class Container uses occupied MetaData! (" +
            classContainer.getMetaId() + ")");
        tFailed = true;
      }
    }
    if (tFailed) {
      ...
      for (StackTraceElement tElement : new Exception().getStackTrace())
        if (i++ < 5 && !tElement.getClassName().startsWith("sun"))
  ...
  public String getLocal(int metaId) {
    return StatCollector.translateToLocal(internalName + "." + metaId +
                                          ".name");
  }
  ...
  public MultiTileEntity getNewTileEntity(final int metaId) {
    ...
    if (clazz == null)
      return null;
    MultiTileEntity te = (MultiTileEntity)GT_Utility.callConstructor(
        clazz.getClazz(), 0, null, true);
  ...
  public MultiTileEntityBlock getBlock() { return block; }
}
