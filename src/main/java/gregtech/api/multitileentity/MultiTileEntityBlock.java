 ...
 */
@Optional.
Interface(iface = "com.cricketcraft.chisel.api.IFacade", modid = "ChiselAPI")
public class MultiTileEntityBlock extends BlockContainer implements IFacade {

  private String toolName;
  private MultiTileEntityRegistry registry;

  public MultiTileEntityBlock() { super(Material.anvil); }

  public MultiTileEntityRegistry getRegistry() { return registry; }

  public MultiTileEntityBlock
  setRegistry(final MultiTileEntityRegistry registry) {
  ...
  public int getRenderType() {
    return GT_MultiTile_Renderer.INSTANCE == null
        ? super.getRenderType()
        : GT_MultiTile_Renderer.INSTANCE.getRenderId();
  }

  @Override
  public void onNeighborBlockChange(final World worldIn, final int x,
  ...
  public boolean onBlockActivated(World worldIn, int x, int y, int z,
                                  EntityPlayer player, int side, float subX,
                                  float subY, float subZ) {
    TileEntity te = worldIn.getTileEntity(x, y, z);
    if (!(te instanceof IMultiTileEntity mute))
      return false;

    return mute.onBlockActivated(player, ForgeDirection.getOrientation(side),
                                 subX, subY, subZ);
  }
  ...
  public void getSubBlocks(final Item itemIn, final CreativeTabs tab,
    ...
    for (final MultiTileEntityClassContainer container :
  ...
  @Override
  public Block getFacade(final IBlockAccess aWorld, final int aX, final int aY,
                         final int aZ, final int ordinalSide) {
    final TileEntity tTileEntity = aWorld.getTileEntity(aX, aY, aZ);
    if (tTileEntity instanceof final CoverableTileEntity tile) {
      final ForgeDirection side = ForgeDirection.getOrientation(ordinalSide);
      if (ordinalSide != -1) {
        final Block facadeBlock =
            tile.getCoverInfoAtSide(side).getFacadeBlock();
        if (facadeBlock != null)
          return facadeBlock;
      } else {
        // we do not allow more than one type of facade per block, so no need to
        // check every side see comment in
        // gregtech.common.covers.GT_Cover_FacadeBase.isCoverPlaceable
        for (final ForgeDirection tSide : ForgeDirection.VALID_DIRECTIONS) {
          final Block facadeBlock =
              tile.getCoverInfoAtSide(tSide).getFacadeBlock();
          if (facadeBlock != null) {
            return facadeBlock;
          }
        }
  ...
  @Override
  public int getFacadeMetadata(final IBlockAccess aWorld, final int aX,
                               final int aY, final int aZ,
  ...
  @Override
  public void breakBlock(World worldIn, int x, int y, int z, Block blockBroken,
                         int meta) {
    final TileEntity te = worldIn.getTileEntity(x, y, z);
    if (te instanceof IMultiTileEntity mute) {
      mute.onBlockBroken();
