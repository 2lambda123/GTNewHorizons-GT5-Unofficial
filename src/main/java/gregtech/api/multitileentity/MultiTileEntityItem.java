...
public class MultiTileEntityItem
    extends ItemBlock implements IFluidContainerItem, IItemUpdatable {
  ...
  public void addInformation(ItemStack stack, EntityPlayer player,
                             List<String> list, boolean aF3_H) {
    MultiTileEntity te = block.getRegistry().getCachedTileEntity(stack);
    if (te != null) {
      te.addToolTip(list);
    }
  ...
  public boolean onItemUse(ItemStack stack, EntityPlayer player, World world,
                           int x, int y, int z, int ordinalSide, float aHitX,
    ...
    try {
      ...
      if (clickedBlock instanceof BlockSnow &&
          (world.getBlockMetadata(x, y, z) & 7) < 1) {
        ordinalSide = SIDE_TOP;
        side = ForgeDirection.UP;
      } else if (clickedBlock != Blocks.vine &&
                 clickedBlock != Blocks.tallgrass &&
      ...
      if (!replacedBlock.isReplaceable(world, x, y, z) ||
          !block.canReplace(world, x, y, z, ordinalSide, stack)) {
        return false;
      }

      if (stack.stackSize == 0 ||
          (!player.canPlayerEdit(x, y, z, ordinalSide, stack))) {
        return false;
      }
      ...
      try {
        if (!world.isRemote) {
          ...
          world.func_147453_f(x, y, z,
                              block); /* updateNeighborsAboutBlockChange */
        }
      } catch (Throwable e) {
  ...
  @Override
  public void updateItemStack(ItemStack aStack, World aWorld, int aX, int aY,
                              int aZ) {}
  ...
  public FluidStack getFluid(ItemStack aStack) {
    MultiTileEntity te = block.getRegistry().getCachedTileEntity(aStack);
  ...
  public FluidStack drain(ItemStack aStack, int aMaxDrain, boolean aDoDrain) {
    ...
    if (te instanceof IFluidContainerItem fluidContainerItem) {
      final FluidStack rFluid =
          fluidContainerItem.drain(aStack, aMaxDrain, aDoDrain);
      updateItemStack(aStack);
      return rFluid;
    }
    return null;
