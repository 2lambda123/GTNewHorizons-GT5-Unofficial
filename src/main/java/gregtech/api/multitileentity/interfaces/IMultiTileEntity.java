...
public interface IMultiTileEntity {
  ...
  void initFromNBT(@Nonnull NBTTagCompound nbt, final int registryId,
                   final int metaId);

  @Nonnull ChunkCoordinates getCoords();

  @Nonnull String getTileEntityName();
  ...
  default boolean onBlockActivated(@Nonnull EntityPlayer player,
                                   ForgeDirection side, float subX, float subY,
                                   float subZ) {
    return false;
  }
