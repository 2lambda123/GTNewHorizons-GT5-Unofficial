...
public abstract class TickableMultiTileEntity
    extends MultiTileEntity implements TaskHost {
  ...
  public TickableMultiTileEntity() { super(true); }
  ...
  /**
   * Gets called when there is an Exception/Error happening during one of the
   * Tick methods.
   */
  ...
  public final void registerTask(@Nonnull TickableTask<?> task) {
    if (tasks.containsKey(task.getName())) {
      throw new IllegalStateException(String.format(
          "Task with name %s is already registered", task.getName()));
    }
