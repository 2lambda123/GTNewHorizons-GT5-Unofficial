...
public class MultiTileEntityClassContainer {
  ...
  public MultiTileEntityClassContainer(MultiTileEntityRegistry registry,
                                       MultiTileEntityBlock block, int metaId,
  ...
  public Class<? extends MultiTileEntity> getClazz() { return clazz; }

  public MultiTileEntity getOriginalTileEntity() { return originalTileEntity; }

  public String getUnlocalizedName() { return unlocalizedName; }

  public NBTTagCompound getParameters() { return parameters; }

  public boolean register() {
    /* End and register the Builder with the registry */
    final MultiTileEntityRegistry registry = this.registry.get();

    if (parameters.hasKey(NBT.MATERIAL) && !parameters.hasKey(NBT.COLOR))
      parameters.setInteger(
          NBT.COLOR,
          GT_Util.getRGBInt(
              Materials.get(parameters.getString(NBT.MATERIAL)).getRGBA()));
    ...
    originalTileEntity.initFromNBT(parameters, Block.getIdFromBlock(block),
                                   metaId);
  ...
  // Need a base texture for the MTE machine, and then a separate texture set
  // for the machine/active overlays
  ...
  public MultiTileEntityClassContainer material(Materials material) {
    ...
    if (!parameters.hasKey(NBT.COLOR))
      parameters.setInteger(NBT.COLOR, GT_Util.getRGBInt(material.getRGBA()));
    return this;
  }
  ...
  public MultiTileEntityClassContainer tier(int aTier) {
    verifyDescendentOfMultiple(true, UpgradeCasing.class,
                               FunctionalCasing.class);
  ...
  public MultiTileEntityClassContainer setNBT(Tuple... aTags) {
    /*
     * Merge in arbitrary NBT tuples of (key, value). Useful for anything for
     * which a custom method has not yet been exposed
     */
  ...
  private void verifyDescendentOf(Class<?> cls) {
    ...
    if (!cls.isAssignableFrom(clazz)) {
      throw new IllegalArgumentException("Expected a descendent of " +
                                         cls.getName() + " got " +
                                         clazz.getName() + " instead.");
    }
  }

  private void verifyDescendentOfMultiple(boolean onlyOne,
                                          Class<?>... classes) {
    ...
    if (!atLeastOne) {
      throw new IllegalArgumentException(
          "Expected a descendent of any of these " + classNames + " got " +
          clazz.getName() + " instead.");
    }
  }
}
