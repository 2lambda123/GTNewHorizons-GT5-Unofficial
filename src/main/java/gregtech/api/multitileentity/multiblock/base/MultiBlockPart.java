...
public abstract class MultiBlockPart extends NonTickableMultiTileEntity
  ...
  @Nonnull
  private static final ITexture OVERLAY_PIPE_IN_TEXTURE =
      TextureFactory.of(OVERLAY_PIPE_IN);
  @Nonnull
  private static final ITexture OVERLAY_PIPE_OUT_TEXTURE =
      TextureFactory.of(OVERLAY_PIPE_OUT);

  protected final List<PartMode> BASIC_MODES = new ArrayList<>(
      Arrays.asList(NOTHING, ENERGY_INPUT, ENERGY_OUTPUT, FLUID_INPUT,
                    FLUID_OUTPUT, ITEM_INPUT, ITEM_OUTPUT));
  ...
  protected int allowedModes =
      NOTHING.getValue();            // BITMASK - Modes allowed for this part
  ...
  @Nonnull protected final GUIProvider<?> guiProvider = createGUIProvider();

  /**
   * What Part Tier is this part? All Basic Casings are Tier 1, and will allow:
   * Energy, Item, Fluid input/output. Some of the more advanced modes can be
   * set to require a higher tier part.
   */
  public int getPartTier() { return 1; }
  ...
  public void setTarget(final IMultiBlockController newTarget,
                        final int aAllowedModes) {
  ...
  public IMultiBlockController getTarget(final boolean checkValidity) {
    ...
    if (!worldObj.blockExists(targetPosition.posX, targetPosition.posY,
                              targetPosition.posZ)) {
      return null;
    }
    final TileEntity te = worldObj.getTileEntity(
        targetPosition.posX, targetPosition.posY, targetPosition.posZ);
  ...
  public void readFromNBT(final NBTTagCompound aNBT) {
    ...
    if (aNBT.hasKey(NBT.ALLOWED_MODES))
      allowedModes = aNBT.getInteger(NBT.ALLOWED_MODES);
    if (aNBT.hasKey(NBT.MODE))
      setMode(PartMode.values()[aNBT.getInteger(NBT.MODE)]);
    if (aNBT.hasKey(NBT.TARGET)) {
      targetPosition = new ChunkCoordinates(aNBT.getInteger(NBT.TARGET_X),
  ...
  public void writeToNBT(final NBTTagCompound aNBT) {
    ...
    if (allowedModes != NOTHING.getValue())
      aNBT.setInteger(NBT.ALLOWED_MODES, allowedModes);
    if (mode.getValue() != 0)
      aNBT.setInteger(NBT.MODE, mode.ordinal());
    if (targetPosition != null) {
  ...
  public int getAllowedModes() { return allowedModes; }
  ...
  /**
   * Returns true if the part has any of the modes provided, and that mode is
  ...
  public void onBlockPlaced() {
    for (final ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
      final TileEntity te = WorldHelper.getTileEntityAtSide(
          side, getWorldObj(), getXCoord(), getYCoord(), getZCoord());
  ...
  protected ITexture getModeTexture() {
    ...
    protected PartMode getNextAllowedMode(final List<PartMode> allowedModes) {
        ...
        for (PartMode newMode : allowedModes) {
                if (newMode.getValue() > getMode().getValue())
                  return newMode;
              }
              // Nothing valid found
              return NOTHING;
            ...
            /**
             * TODO: Make sure the energy/item/fluid hatch is facing that way!
             * or has that mode enabled on that side Check SIDE_UNKNOWN for or
             * coverbehavior
             */

            // #region Fluid - Depending on the part type - proxy it to the
            // multiblock controller, if we have one
            @Override
            @Nullable
            public FluidInventoryLogic getFluidLogic(
              ...
              if (controller == null)
                return null;
              return controller.getFluidLogic(modeSelected(FLUID_INPUT)
                                                  ? InventoryType.Input
            ...
            // #region Energy - Depending on the part type - proxy to the
            // multiblock controller, if we have one
            ...
            // #region Item - Depending on the part type - proxy to the
            // multiblock controller, if we have one
            ...
            public ItemInventoryLogic getItemLogic(
                @Nonnull final ForgeDirection side,
                @Nonnull final InventoryType unused) {
              if (side != getFacing() && side != ForgeDirection.UNKNOWN)
                return null;
              ...
              return controller.getItemLogic(modeSelected(ITEM_INPUT)
                                                 ? InventoryType.Input
                                                 : InventoryType.Output,
                                             lockedInventory);
            }
            ...
            public InventoryType getItemInventoryType() {
              ...
              return modeSelected(ITEM_INPUT) ? InventoryType.Input
                                              : InventoryType.Output;
            }
            ...
            public String getLocalName() {
              if (modeSelected(ITEM_INPUT))
                return "Input Inventory";
              if (modeSelected(ITEM_OUTPUT))
                return "Output Inventory";
              if (modeSelected(FLUID_INPUT))
            ...
            protected Fluid getLockedFluid() {
              if (configurationTank.get() != null &&
                  configurationTank.get().getFluid() != null) {
                return configurationTank.get().getFluid();
              }
              return null;
            }

            protected boolean canOpenControllerGui() { return true; }

            public void addToolTips(final List<String> list,
                                    final ItemStack stack, final boolean f3_h) {
            ...
            public String getInventoryName() {
              ...
              if (controller == null)
                return "";
              if (modeSelected(ITEM_INPUT, ITEM_OUTPUT)) {
                final InventoryType type = modeSelected(ITEM_INPUT)
                                               ? InventoryType.Input
                                               : InventoryType.Output;
                final ItemInventoryLogic itemLogic =
                    controller.getItemLogic(type, lockedInventory);
                return itemLogic.getDisplayName();
              }
              if (modeSelected(FLUID_INPUT, FLUID_OUTPUT)) {
                final InventoryType type = modeSelected(FLUID_INPUT)
                                               ? InventoryType.Input
                                               : InventoryType.Output;
                final FluidInventoryLogic fluidLogic =
                    controller.getFluidLogic(type, lockedInventory);
            ...
            public GUIProvider<?> getGUI(
                @Nonnull final UIBuildContext uiContext) {
              final IMultiBlockController controller = getTarget(false);
              if (controller == null)
                return guiProvider;
              if (!modeSelected(NOTHING, ENERGY_INPUT, ENERGY_OUTPUT))
                return guiProvider;
              if (!canOpenControllerGui())
                return guiProvider;
              if (uiContext.getPlayer().isSneaking())
                return guiProvider;
            ...
            public String getMachineName() {
              return StatCollector.translateToLocal(
                  getAsItem().getUnlocalizedName());
            }

            @Override
            protected boolean onRightClick(EntityPlayer player,
                                           ForgeDirection side,
            ...
            @Override
            protected boolean onRightClickWithMallet(
                EntityPlayer player, ForgeDirection side,
                ForgeDirection wrenchSide) {
              if (allowedModes == NOTHING.getValue())
                return true;
              setMode(getNextAllowedMode(
                  PartMode.getPartModesFromBitmask(allowedModes)));
                ...
                GTNHLib.proxy.sendMessageAboveHotbar(
                    ...
                    new ChatComponentText(
                        ...
                        StatCollector.translateToLocal(
                            "gt.multiblockpart.setmode.text") +
