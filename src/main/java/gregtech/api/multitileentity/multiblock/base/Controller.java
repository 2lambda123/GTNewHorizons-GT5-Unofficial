...
/**
 * Multi Tile Entities - or MuTEs - don't have dedicated hatches, but their
 * casings can become hatches.
 */
public abstract class Controller<C extends Controller<C, P>, P
                                     extends MuTEProcessingLogic<P>>
    extends MultiTileBasicMachine<P>
    implements IAlignment, IMultiBlockController, IDescribable,
               ISurvivalConstructable {

  public static final String ALL_INVENTORIES_NAME = "all";
  protected static final int AUTO_OUTPUT_FREQUENCY_TICK = 20;

  private static final Map<Integer, GT_Multiblock_Tooltip_Builder> tooltip =
  ...
  protected List<LinkedList<WeakReference<IMultiBlockPart>>>
      registeredCoveredParts =
          Arrays.asList(new LinkedList<>(), new LinkedList<>(),
  ...
  /**
   * Due to limitation of Java type system, you might need to do an unchecked
   * cast. HOWEVER, the returned IStructureDefinition is expected to be
   * evaluated against current instance only, and should not be used against
   * other instances, even for those of the same class.
   */
  @Override public abstract IStructureDefinition<C> getStructureDefinition();

  /**
   * Checks the Machine.
   * <p>
   * NOTE: If using `buildState` be sure to `startBuilding()` and either
   * `endBuilding()` or `failBuilding()`
   */
  ...
  public void readFromNBT(NBTTagCompound nbt) {
    ...
    // Multiblock inventories are a collection of inventories. The first
    // inventory is the default internal inventory, and the others are added by
    // inventory extending blocks.

    structureOkay = nbt.getBoolean(NBT.STRUCTURE_OK);
    extendedFacing = ExtendedFacing.of(
        getFacing(), Rotation.byIndex(nbt.getByte(NBT.ROTATION)),
   ...
   * <p>
   * Imagine you stand in front of the controller, with controller facing
   * towards you not rotated or flipped. <p> The horizontalOffset would be the
   * number of blocks on the left side of the controller, not counting
   * controller itself. The verticalOffset would be the number of blocks on the
   * top side of the controller, not counting controller itself. The depthOffset
   * would be the number of blocks between you and controller, not counting
   * controller itself.
   * <p>
   * All these offsets can be negative.
   */
  protected final boolean checkPiece(String piece, int horizontalOffset,
                                     int verticalOffset, int depthOffset) {
    return getCastedStructureDefinition().check(
        this, piece, getWorldObj(), getExtendedFacing(), getXCoord(),
        getYCoord(), getZCoord(), horizontalOffset, verticalOffset, depthOffset,
        !structureOkay);
  }

  public final boolean buildPiece(String piece, ItemStack trigger,
                                  boolean hintsOnly, Vec3Impl offset) {
    return buildPiece(piece, trigger, hintsOnly, offset.get0(), offset.get1(),
                      offset.get2());
  }

  protected final boolean buildPiece(String piece, ItemStack trigger,
                                     boolean hintOnly, int horizontalOffset,
                                     int verticalOffset, int depthOffset) {
    return getCastedStructureDefinition().buildOrHints(
        this, trigger, piece, getWorldObj(), getExtendedFacing(), getXCoord(),
        getYCoord(), getZCoord(), horizontalOffset, verticalOffset, depthOffset,
        hintOnly);
  }

  protected final int survivalBuildPiece(String piece, ItemStack trigger,
                                         Vec3Impl offset, int elementBudget,
                                         ISurvivalBuildEnvironment env,
                                         boolean check) {
    return survivalBuildPiece(piece, trigger, offset.get0(), offset.get1(),
                              offset.get2(), elementBudget, env, check);
  }

  protected final Integer survivalBuildPiece(String piece, ItemStack trigger,
                                             int horizontalOffset,
                                             int verticalOffset,
                                             int depthOffset, int elementBudget,
                                             ISurvivalBuildEnvironment env,
                                             boolean check) {
    return getCastedStructureDefinition().survivalBuild(
        this, trigger, piece, getWorldObj(), getExtendedFacing(), getXCoord(),
        getYCoord(), getZCoord(), horizontalOffset, verticalOffset, depthOffset,
        elementBudget, env, check);
  }

  @SuppressWarnings("unchecked")
  private IStructureDefinition<Controller<C, P>>
  ...
  public void setExtendedFacing(ExtendedFacing newExtendedFacing) {
    ...
    if (isServerSide()) {
      StructureLibAPI.sendAlignment(
          this, new NetworkRegistry.TargetPoint(
                    getWorldObj().provider.dimensionId, getXCoord(),
                    getYCoord(), getZCoord(), 512));
    } else {
      issueClientUpdate();
    }
  }
  ...
  @Override
  public void registerCoveredPartOnSide(final ForgeDirection side,
                                        IMultiBlockPart part) {
    if (side == ForgeDirection.UNKNOWN)
      return;

    final LinkedList<WeakReference<IMultiBlockPart>> registeredCovers =
        registeredCoveredParts.get(side.ordinal());
    // TODO: Make sure that we're not already registered on this side
    registeredCovers.add(new WeakReference<>(part));
  }

  @Override
  public void unregisterCoveredPartOnSide(final ForgeDirection side,
                                          IMultiBlockPart aPart) {
    if (side == ForgeDirection.UNKNOWN)
      return;

    final LinkedList<WeakReference<IMultiBlockPart>> coveredParts =
        registeredCoveredParts.get(side.ordinal());
    final Iterator<WeakReference<IMultiBlockPart>> it = coveredParts.iterator();
    while (it.hasNext()) {
      final IMultiBlockPart part = (it.next()).get();
      if (part == null || part == aPart)
        it.remove();
    }
  }

  @Override
  public void registerCaseWithPurpose(MultiTileCasingPurpose purpose,
                                      IMultiBlockPart part) {
    final LinkedList<WeakReference<IMultiBlockPart>> tickableParts =
        registeredTickableParts.get(purpose.ordinal());
    final Iterator<WeakReference<IMultiBlockPart>> it =
        tickableParts.iterator();
    while (it.hasNext()) {
  ...
  @Override
  public void unregisterCaseWithPurpose(MultiTileCasingPurpose purpose,
                                        IMultiBlockPart part) {
    final LinkedList<WeakReference<IMultiBlockPart>> tickableParts =
        registeredTickableParts.get(purpose.ordinal());
    final Iterator<WeakReference<IMultiBlockPart>> it =
  ...
  private boolean tickCovers() {
    for (final ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
      ...
      final LinkedList<WeakReference<IMultiBlockPart>> coveredParts =
          this.registeredCoveredParts.get(side.ordinal());
      final Iterator<WeakReference<IMultiBlockPart>> it =
          coveredParts.iterator();
      while (it.hasNext()) {
        final IMultiBlockPart part = (it.next()).get();
        if (part == null) {
          it.remove();
  ...
  protected void pushItemOutputs(long tick) {
    if (tick % AUTO_OUTPUT_FREQUENCY_TICK != 0)
      return;
    final LinkedList<WeakReference<IMultiBlockPart>> registeredItemOutputs =
        registeredTickableParts.get(
            MultiTileCasingPurpose.ItemOutput.ordinal());
    final Iterator<WeakReference<IMultiBlockPart>> itemOutputIterator =
        registeredItemOutputs.iterator();
    while (itemOutputIterator.hasNext()) {
      ...
      final IInventory facingInventory = WorldHelper.getIInventoryAtSide(
          part.getFacing(), getWorldObj(), coords.posX, coords.posY,
          coords.posZ);
      if (facingInventory == null) {
        continue;
      }

      moveMultipleItemStacks(part, facingInventory, part.getFacing(),
                             part.getFacing().getOpposite(), null, false,
                             (byte)64, (byte)1, (byte)64, (byte)1,
                             part.getSizeInventory());
      for (int i = 0; i < part.getSizeInventory(); i++) {
        if (part.getStackInSlot(i) != null &&
            part.getStackInSlot(i).stackSize <= 0) {
  ...
  protected void pushFluidOutputs(long tick) {
    if (tick % AUTO_OUTPUT_FREQUENCY_TICK != 0)
      return;
    final LinkedList<WeakReference<IMultiBlockPart>> registeredFluidOutputs =
        registeredTickableParts.get(
            MultiTileCasingPurpose.FluidOutput.ordinal());
    final Iterator<WeakReference<IMultiBlockPart>> fluidOutputIterator =
        registeredFluidOutputs.iterator();
  ...
  public boolean isSeparateInputs() { return separateInputs; }
  ...
  @Override
  @Nullable
  public FluidInventoryLogic getFluidLogic(@Nonnull ForgeDirection side,
  ...
  @Override
  @Nullable
  public ItemInventoryLogic getItemLogic(@Nonnull ForgeDirection side,
                                         @Nonnull InventoryType type) {
    if (side == getFacing())
      return null;
    return switch (type) {
  ...
  protected void updateSlots() {
    controllerItemInput.getAllInventoryLogics().update(shouldSort);
    controllerItemOutput.getAllInventoryLogics().update(shouldSort);
    controllerFluidInput.getAllInventoryLogics().update();
    controllerFluidOutput.getAllInventoryLogics().update();
  }
  ...
  @Override
  public List<? extends IFluidStore>
  getFluidOutputSlots(FluidStack[] toOutput) {
    return new ArrayList<>(0);
  }

  @Override
  @Nonnull
  public Set<Entry<UUID, FluidInventoryLogic>>
  getAllFluidInventoryLogics(@Nonnull InventoryType type) {
    return switch (type) {
