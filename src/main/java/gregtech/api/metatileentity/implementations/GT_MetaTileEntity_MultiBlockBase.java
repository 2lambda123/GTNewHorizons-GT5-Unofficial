...
public abstract class GT_MetaTileEntity_MultiBlockBase extends MetaTileEntity
    implements ControllerWithOptionalFeatures, IAddGregtechLogo, IAddUIWidgets,
               IBindPlayerInventoryUI {

  public static boolean disableMaintenance;
  public boolean mMachine = false, mWrench = false, mScrewdriver = false,
                 mSoftHammer = false, mHardHammer = false,
                 mSolderingTool = false, mCrowbar = false,
                 mRunningOnLoad = false;
  public boolean mStructureChanged = false;
  public int mPollution = 0, mProgresstime = 0, mMaxProgresstime = 0, mEUt = 0,
             mEfficiencyIncrease = 0, mStartUpCheck = 100, mRuntime = 0,
             mEfficiency = 0;
  ...
  private @Nonnull CheckRecipeResult checkRecipeResult =
      CheckRecipeResultRegistry.NONE;
  ...
  public GT_MetaTileEntity_MultiBlockBase(int aID, String aName,
    ...
    GT_MetaTileEntity_MultiBlockBase.disableMaintenance =
        GregTech_API.sMachineFile.get(ConfigCategories.machineconfig,
                                      "MultiBlockMachines.disableMaintenance",
                                      false);
    this.damageFactorLow =
        GregTech_API.sMachineFile.get(ConfigCategories.machineconfig,
                                      "MultiBlockMachines.damageFactorLow", 5);
    this.damageFactorHigh = (float)GregTech_API.sMachineFile.get(
        ConfigCategories.machineconfig, "MultiBlockMachines.damageFactorHigh",
        0.6f);
    this.mNEI = "";
  }
  ...
  @Override
  public void onScrewdriverRightClick(ForgeDirection side, EntityPlayer aPlayer,
    ...
    if (supportsSingleRecipeLocking()) {
      ...
      if (mLockedToSingleRecipe) {
        ...
        GT_Utility.sendChatToPlayer(
            aPlayer,
            GT_Utility.trans("220", "Single recipe locking disabled."));
  ...
  public void loadNBTData(NBTTagCompound aNBT) {
    ...
    if (supportsSingleRecipeLocking()) {
      ...
      if (mLockedToSingleRecipe &&
          aNBT.hasKey("mSingleRecipeCheck", Constants.NBT.TAG_COMPOUND)) {
        SingleRecipeCheck c =
            loadSingleRecipeChecker(aNBT.getCompoundTag("mSingleRecipeCheck"));
        if (c != null)
          mSingleRecipeCheck = c;
        // the old recipe is gone. we disable the machine to prevent making
        // garbage in case of shared inputs maybe use a better way to inform
    ...
    if (aNBT.hasKey(VOIDING_MODE_NBT_KEY, Constants.NBT.TAG_STRING)) {
      ...
      voidingMode = aNBT.getBoolean(VOID_EXCESS_NBT_KEY)
                        ? VoidingMode.VOID_ALL
                        : VoidingMode.VOID_NONE;
    }
    if (!getAllowedVoidingModes().contains(voidingMode))
   ...
   */
  public void onStructureChange() { mStructureChanged = true; }
  ...
  /**
   * ClearHatches as a part of structure check. If your multiblock has any
   * hatches that need clearing override this method, call super, and clear your
   * own hatches
   */
  ...
  public boolean checkStructure(boolean aForceReset,
    ...
    if (!aBaseMetaTileEntity.isServerSide())
      return mMachine;
    // Only trigger an update if forced (from onPostTick, generally), or if the
  ...
  public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
    if (aBaseMetaTileEntity.isServerSide()) {
      if (mEfficiency < 0)
        mEfficiency = 0;
      if (mUpdated) {
        // duct tape fix for too many updates on an overloaded server, causing
        // the structure check to not run
        if (mUpdate <= 0)
          mUpdate = 50;
        mUpdated = false;
      }
  ...
  private void checkMaintenance() {
    ...
    for (GT_MetaTileEntity_Hatch_Maintenance tHatch :
         filterValidMTEs(mMaintenanceHatches)) {
  ...
  /**
   * Starts checking recipe with some operations needed to actually run the
   * check. Overriding this without due care may result in dupe of items, hence
   * it's marked as final. <p> See {@link #createProcessingLogic()} or {@link
   * #checkProcessing()} for what you want to override.
   *
  ...
  protected final boolean checkRecipe() {
    ...
    if (!CheckRecipeResultRegistry.isRegistered(result.getID())) {
      throw new RuntimeException(String.format(
          "Result %s is not registered for registry", result.getID()));
    }
    ...
    endRecipeProcessing();
    // Don't use `result` here because `endRecipeProcessing()` might mutate
    // `this.checkRecipeResult`
  ...
  private boolean shouldCheckRecipeThisTick(long aTick) {
    ...
    // check all of them (i.e. do not return early) to reset the state of all of
    // them.
    ...
    if (!isBatchModeEnabled()) {
      return timeElapsed == 5 || timeElapsed == 12 || timeElapsed == 20 ||
          timeElapsed == 30 || timeElapsed == 40 || timeElapsed == 55 ||
          timeElapsed == 70 || timeElapsed == 85;
    }
    return false;
  }
  ...
  protected void runMachine(IGregTechTileEntity aBaseMetaTileEntity,
    ...
    if (mMaxProgresstime > 0 && doRandomMaintenanceDamage()) {
      ...
      if (shouldCheckRecipeThisTick(aTick) ||
  ...
  public boolean polluteEnvironment(int aPollutionLevel) {
    ...
    for (GT_MetaTileEntity_Hatch_Muffler tHatch :
         filterValidMTEs(mMufflerHatches)) {
      if (mPollution >= 10000) {
  ...
  public void doSound(byte aIndex, double aX, double aY, double aZ) {
    ...
    public CheckRecipeResult checkProcessing() {
        ...
        if (processingLogic == null) {
            ...
            mEfficiency =
                (10000 - (getIdealStatus() - getRepairStatus()) * 1000);
  ...
  /**
   * @return If controller slot should be considered as inputs for {@link
   *     #doCheckRecipe}
   */
  protected boolean canUseControllerSlotForRecipe() { return true; }

  /**
   * Initializes processing logic for use. Unlike {@link
   * #createProcessingLogic}, this method is called every time checking for
   * recipes.
   */
  ...
  /**
   * Initializes processing logic for use, specifically for power-related
   * parameters. Unlike {@link #createProcessingLogic}, this method is called
  ...
  protected boolean supportsCraftingMEBuffer() { return true; }

  /**
   * Iterates over hatches and tries to find recipe. Assume {@link
   * #processingLogic} is already set up for use. If return value is successful,
   * inputs are consumed.
   */
  @Nonnull
      ...
      for (IDualInputHatch dualInputHatch : mDualInputHatches) {
        for (var it = dualInputHatch.inventories(); it.hasNext();) {
          ...
          if (foundResult != CheckRecipeResultRegistry.NO_RECIPE) {
            // Recipe failed in interesting way, so remember that and continue
            // searching
            result = foundResult;
          }
        }
  ...
  /**
   * Performs additional check for {@link #processingLogic} after all the
   * calculations are done. As many as checks should be done inside of custom
   * {@link ProcessingLogic}, which you can specify with
   * {@link #createProcessingLogic()}, because when this method is called,
   * inputs might have been already consumed. However, certain checks cannot be
   * done like that; Checking energy overflow should be suppressed for
   * long-power machines for example.
   *
   * @return Modified (or not modified) result
   */
  @Nonnull
  protected CheckRecipeResult
  postCheckRecipe(@Nonnull CheckRecipeResult result,
                  @Nonnull ProcessingLogic processingLogic) {
    if (result.wasSuccessful() &&
        processingLogic.getCalculatedEut() > Integer.MAX_VALUE) {
  ...
  /**
   * Called after {@link #doCheckRecipe} and {@link #postCheckRecipe} being
   * successful. Override to set energy usage for this machine.
   */
  ...
  protected int getMaxBatchSize() { return 128; }

  /**
   * Checks the Machine. You have to assign the MetaTileEntities for the Hatches
   * here.
   */
  public abstract boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity,
                                       ItemStack aStack);
  ...
  /**
   * Gets the pollution produced per second by this multiblock, default to 0.
   * Override this with its actual value in the code of the multiblock.
   */
  public int getPollutionPerSecond(ItemStack aStack) { return 0; }
  ...
  public void stopMachine(@Nonnull ShutDownReason reason) {
    if (!ShutDownReasonRegistry.isRegistered(reason.getID())) {
      throw new RuntimeException(String.format(
          "Reason %s is not registered for registry", reason.getID()));
    }
    mOutputItems = null;
    mOutputFluids = null;
    mEUt = 0;
  ...
  public int getRepairStatus() {
    return (mWrench ? 1 : 0) + (mScrewdriver ? 1 : 0) + (mSoftHammer ? 1 : 0) +
        (mHardHammer ? 1 : 0) + (mSolderingTool ? 1 : 0) + (mCrowbar ? 1 : 0);
  }

  public int getIdealStatus() { return 6; }
  ...
  public boolean doRandomMaintenanceDamage() {
    ...
    public void explodeMultiblock() {
        ...
        for (MetaTileEntity tTileEntity : allHatches) {
                        if (tTileEntity != null &&
                            tTileEntity.getBaseMetaTileEntity() != null) {
                    ...
                    public boolean addEnergyOutputMultipleDynamos(
                        long aEU, boolean aAllowMixedVoltageDynamos) {
                      int injected = 0;
                      long totalOutput = 0;
                      long aFirstVoltageFound = -1;
                      boolean aFoundMixedDynamos = false;
                      for (GT_MetaTileEntity_Hatch_Dynamo aDynamo :
                           filterValidMTEs(mDynamoHatches)) {
                      ...
                      if (totalOutput < aEU ||
                          (aFoundMixedDynamos && !aAllowMixedVoltageDynamos)) {
                        explodeMultiblock();
                        return false;
                      }
                      ...
                      for (GT_MetaTileEntity_Hatch_Dynamo aDynamo :
                        ...
                        aRemainder =
                            (int)(leftToInject - (aAmpsToInject * aVoltage));
                        ampsOnCurrentHatch = (int)Math.min(
                            aDynamo.maxAmperesOut(), aAmpsToInject);
                        ...
                        if (aRemainder > 0 &&
                            ampsOnCurrentHatch < aDynamo.maxAmperesOut()) {
                          aDynamo.getBaseMetaTileEntity()
                    ...
                    /**
                     * Sums up voltage of energy hatches. Amperage does not
                     * matter.
                     */
                    public long getMaxInputVoltage() {
                      long rVoltage = 0;
                      for (GT_MetaTileEntity_Hatch_Energy tHatch :
                           filterValidMTEs(mEnergyHatches))
                        rVoltage +=
                            tHatch.getBaseMetaTileEntity().getInputVoltage();
                      return rVoltage;
                    }

                    public long getAverageInputVoltage() {
                      return GT_ExoticEnergyInputHelper
                          .getAverageInputVoltageMulti(mEnergyHatches);
                    }

                    public long getMaxInputAmps() {
                      return GT_ExoticEnergyInputHelper
                          .getMaxWorkingInputAmpsMulti(mEnergyHatches);
                    }

                    public long getMaxInputEu() {
                      return GT_ExoticEnergyInputHelper.getTotalEuMulti(
                          mEnergyHatches);
                    }

                    /**
                     * Sums up max input EU/t of energy hatches, amperage
                     * included.
                     */
                    public long getMaxInputPower() {
                      ...
                      for (GT_MetaTileEntity_Hatch_Energy tHatch :
                        ...
                        eut += baseTile.getInputVoltage() *
                               baseTile.getInputAmperage();
                      }
                      return eut;
                    }

                    /**
                     * Returns voltage tier of energy hatches. If multiple tiers
                     * are found, returns 0.
                     */
                    public long getInputVoltageTier() {
                      ...
                      if (mEnergyHatches.size() > 0) {
                        ...
                        for (int i = 1; i < mEnergyHatches.size(); i++) {
                          if (mEnergyHatches.get(i).getInputTier() != rTier)
                            return 0;
                        }
                      }
                     ...
                     *     Voltage is rounded up to higher tier voltage.
                     * @param perfectOC       - If the Multiblock OCs perfectly,
                     *     i.e. the large Chemical Reactor
                     */
                    protected void calculateOverclockedNessMultiInternal(
                        long aEUt, int aDuration, int mAmperage,
                        long maxInputVoltage, boolean perfectOC) {
                      byte tier = (byte)Math.max(
                          0, GT_Utility.getTier(maxInputVoltage));
                      GT_OverclockCalculator calculator =
                          new GT_OverclockCalculator()
                              .setRecipeEUt(aEUt)
                    ...
                    @Deprecated
                    protected void calculateOverclockedNessMulti(
                        int aEUt, int aDuration, int mAmperage,
                        long maxInputVoltage) {
                      calculateOverclockedNessMultiInternal(
                          aEUt, aDuration, mAmperage, maxInputVoltage, false);
                    }
                    ...
                    @Deprecated
                    protected void calculatePerfectOverclockedNessMulti(
                        int aEUt, int aDuration, int mAmperage,
                        long maxInputVoltage) {
                      calculateOverclockedNessMultiInternal(
                          aEUt, aDuration, mAmperage, maxInputVoltage, true);
                    }
                    ...
                    protected static boolean dumpFluid(
                      ...
                      for (GT_MetaTileEntity_Hatch_Output tHatch :
                           filterValidMTEs(aOutputHatches)) {
                        if (restrictiveHatchesOnly && tHatch.mMode == 0) {
                          continue;
                        }
                        if (!tHatch.canStoreFluid(copiedFluidStack))
                          continue;
                        int tAmount = tHatch.fill(copiedFluidStack, false);
                        if (tAmount >= copiedFluidStack.amount) {
                          boolean filled =
                              tHatch.fill(copiedFluidStack, true) >=
                              copiedFluidStack.amount;
                    ...
                    protected void addFluidOutputs(
                        FluidStack[] mOutputFluids2) {
                    ...
                    public boolean depleteInput(FluidStack aLiquid,
                      ...
                      if (aLiquid == null)
                        return false;
                      for (GT_MetaTileEntity_Hatch_Input tHatch :
                           filterValidMTEs(mInputHatches)) {
                        setHatchRecipeMap(tHatch);
                        FluidStack tLiquid = tHatch.drain(
                            ForgeDirection.UNKNOWN, aLiquid, false);
                        if (tLiquid != null &&
                            tLiquid.amount >= aLiquid.amount) {
                          if (simulate) {
                            return true;
                          }
                          tLiquid = tHatch.drain(ForgeDirection.UNKNOWN,
                                                 aLiquid, true);
                          return tLiquid != null &&
                              tLiquid.amount >= aLiquid.amount;
                        }
                    ...
                    public boolean addOutput(ItemStack aStack) {
                      ...
                      for (GT_MetaTileEntity_Hatch_OutputBus tHatch :
                           filterValidMTEs(mOutputBusses)) {
                      ...
                      while (outputSuccess && aStack.stackSize > 0) {
                        ...
                        for (GT_MetaTileEntity_Hatch_Output tHatch :
                             filterValidMTEs(mOutputHatches)) {
                          if (!outputSuccess && tHatch.outputsItems()) {
                            if (tHatch.getBaseMetaTileEntity().addStackToSlot(
                                    1, single))
                              outputSuccess = true;
                          }
                        }
                    ...
                    public boolean depleteInput(ItemStack aStack) {
                      if (GT_Utility.isStackInvalid(aStack))
                        return false;
                      FluidStack aLiquid =
                          GT_Utility.getFluidForFilledItem(aStack, true);
                      if (aLiquid != null)
                      ...
                      for (GT_MetaTileEntity_Hatch_Input tHatch :
                        ...
                        if (GT_Utility.areStacksEqual(
                          ...
                          if (tHatch.getBaseMetaTileEntity()
                            ...
                            tHatch.getBaseMetaTileEntity().decrStackSize(
                                0, aStack.stackSize);
                            return true;
                          }
                        }
                    ...
                    public ArrayList<ItemStack> getStoredOutputs() {
                      ...
                      for (GT_MetaTileEntity_Hatch_OutputBus tHatch :
                        ...
                        for (int i = tHatch.getBaseMetaTileEntity()
                             ...
                             i >= 0; i--) {
                          rList.add(
                              tHatch.getBaseMetaTileEntity().getStackInSlot(i));
                        }
                      }
                      return rList;
                    }
                    ...
                    public ArrayList<FluidStack> getStoredFluids() {
                      ...
                      for (GT_MetaTileEntity_Hatch_Input tHatch :
                        ...
                        if (tHatch instanceof GT_MetaTileEntity_Hatch_MultiInput
                                                  multiInputHatch) {
                          for (FluidStack tFluid :
                               multiInputHatch.getStoredFluid()) {
                            if (tFluid != null) {
                              rList.add(tFluid);
                            }
                          }
                        } else if (tHatch instanceof
                                   GT_MetaTileEntity_Hatch_Input_ME meHatch) {
                          for (FluidStack fluidStack :
                               meHatch.getStoredFluids()) {
                            if (fluidStack != null) {
                              // Prevent the same fluid from different ME
                              // hatches from being recognized
                              inputsFromME.put(fluidStack.getFluid(),
                                               fluidStack);
                            }
                          }
                        } else {
                    ...
                    /**
                     * Drains fluid from the given hatch, including {@link
                     * IDualInputHatch}. Should never be used during recipe
                     * check!
                     *
                     * @param doDrain If false, fluid will not actually be
                     *     consumed
                     * @return Whether the hatch contains enough fluid to drain
                     */
                    public boolean drain(GT_MetaTileEntity_Hatch hatch,
                                         FluidStack fluid, boolean doDrain) {
                      if (fluid == null || hatch == null)
                        return false;
                      if (supportsCraftingMEBuffer() &&
                          hatch instanceof IDualInputHatch tHatch &&
                          tHatch.supportsFluids()) {
                        Optional<IDualInputInventory> inventory =
                            tHatch.getFirstNonEmptyInventory();
                        if (inventory.isPresent()) {
                          for (FluidStack storedFluid : Lists.newArrayList(
                                   inventory.get().getFluidInputs())) {
                            if (fluid.isFluidEqual(storedFluid)) {
                              if (doDrain)
                                storedFluid.amount = Math.max(
                                    storedFluid.amount - fluid.amount, 0);
                      ...
                      if (hatch instanceof
                              GT_MetaTileEntity_Hatch_Input tHatch &&
                          tHatch.isValid()) {
                        if (tHatch instanceof
                            GT_MetaTileEntity_Hatch_Input_ME meHatch) {
                          meHatch.startRecipeProcessing();
                          FluidStack tFluid = meHatch.drain(
                              ForgeDirection.UNKNOWN, fluid, doDrain);
                          meHatch.endRecipeProcessing(this);
                          return tFluid != null &&
                              tFluid.amount >= fluid.amount;
                        } else {
                          FluidStack tFluid = tHatch.drain(
                              ForgeDirection.UNKNOWN, fluid, doDrain);
                          return tFluid != null &&
                              tFluid.amount >= fluid.amount;
                        }
                    ...
                    public ArrayList<ItemStack> getStoredInputs() {
                      ArrayList<ItemStack> rList = new ArrayList<>();
                      Map<GT_Utility.ItemId, ItemStack> inputsFromME =
                          new HashMap<>();
                      for (GT_MetaTileEntity_Hatch_InputBus tHatch :
                           filterValidMTEs(mInputBusses)) {
                        if (tHatch instanceof
                        ...
                        IGregTechTileEntity tileEntity =
                            tHatch.getBaseMetaTileEntity();
                        boolean isMEBus = tHatch instanceof
                                          GT_MetaTileEntity_Hatch_InputBus_ME;
                        for (int i = tileEntity.getSizeInventory() - 1; i >= 0;
                             i--) {
                          ItemStack itemStack = tileEntity.getStackInSlot(i);
                          if (itemStack != null) {
                            if (isMEBus) {
                              // Prevent the same item from different ME buses
                      ...
                      if (getStackInSlot(1) != null &&
                          getStackInSlot(1).getUnlocalizedName().startsWith(
                    ...
                    /**
                     * Anything that is usually separated off in {@link
                     * #getStoredInputs()} (like crafting input bus/buffer) is
                     * also included here.
                     */
                    public ArrayList<ItemStack> getAllStoredInputs() {
                      ...
                      if (supportsCraftingMEBuffer()) {
                        for (IDualInputHatch dualInputHatch :
                             mDualInputHatches) {
                          Iterator<? extends IDualInputInventory>
                              inventoryIterator = dualInputHatch.inventories();
                          while (inventoryIterator.hasNext()) {
                            ItemStack[] items =
                                inventoryIterator.next().getItemInputs();
                            if (items == null) {
                              continue;
                            }
                    ...
                    public Map<GT_Utility.ItemId, ItemStack>
                    getStoredInputsFromME() {
                      Map<GT_Utility.ItemId, ItemStack> inputsFromME =
                          new Object2ReferenceOpenHashMap<>();
                      for (GT_MetaTileEntity_Hatch_InputBus tHatch :
                           filterValidMTEs(mInputBusses)) {
                        if (tHatch instanceof
                            GT_MetaTileEntity_Hatch_InputBus_ME meBus) {
                          for (int i = meBus.getSizeInventory() - 1; i >= 0;
                               i--) {
                    ...
                    public Map<Fluid, FluidStack> getStoredFluidsFromME() {
                      Map<Fluid, FluidStack> fluidsFromME =
                    ...
                    /**
                     * Creates logic to run recipe check based on recipemap.
                     * This runs only once, on class instantiation. <p> If this
                     * machine doesn't use recipemap or does some complex
                     * things, override {@link #checkProcessing()}.
                     */
                    ...
                    protected void startRecipeProcessing() {
                      for (GT_MetaTileEntity_Hatch_InputBus hatch :
                           filterValidMTEs(mInputBusses)) {
                        if (hatch instanceof
                      ...
                      for (GT_MetaTileEntity_Hatch_Input hatch :
                           filterValidMTEs(mInputHatches)) {
                        if (hatch instanceof
                            IRecipeProcessingAwareHatch aware) {
                          aware.startRecipeProcessing();
                    ...
                    public boolean addToMachineList(
                      ...
                      if (aMetaTileEntity instanceof
                        ...
                        setHatchRecipeMap(
                            (GT_MetaTileEntity_Hatch_Input)aMetaTileEntity);
                        return mInputHatches.add(
                            (GT_MetaTileEntity_Hatch_Input)aMetaTileEntity);
                      }
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_InputBus) {
                        ((GT_MetaTileEntity_Hatch_InputBus)aMetaTileEntity)
                            .mRecipeMap = getRecipeMap();
                      ...
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Maintenance)
                        return mMaintenanceHatches.add(
                            (GT_MetaTileEntity_Hatch_Maintenance)
                                aMetaTileEntity);
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Muffler)
                        return mMufflerHatches.add(
                            (GT_MetaTileEntity_Hatch_Muffler)aMetaTileEntity);
                      return false;
                    }

                    public boolean addMaintenanceToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity == null)
                        return false;
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Maintenance hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addEnergyInputToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null) {
                        return false;
                      }
                      ...
                      if (aMetaTileEntity == null)
                        return false;
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Energy hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addExoticEnergyInputToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity instanceof
                              GT_MetaTileEntity_Hatch hatch &&
                          GT_ExoticEnergyInputHelper.isExoticEnergyInput(
                              aMetaTileEntity)) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addDynamoToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity == null)
                        return false;
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Dynamo hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addMufflerToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity == null)
                        return false;
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Muffler hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addInputToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      return addInputBusToMachineList(aTileEntity,
                                                      aBaseCasingIndex) ||
                          addInputHatchToMachineList(aTileEntity,
                                                     aBaseCasingIndex);
                    }

                    public boolean addOutputToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      return addOutputBusToMachineList(aTileEntity,
                                                       aBaseCasingIndex) ||
                          addOutputHatchToMachineList(aTileEntity,
                                                      aBaseCasingIndex);
                    }

                    public boolean addInputBusToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_InputBus hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addOutputBusToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity == null)
                        return false;
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_OutputBus hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addInputHatchToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity == null)
                        return false;
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Input hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    public boolean addOutputHatchToMachineList(
                        IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
                      if (aTileEntity == null)
                        return false;
                      IMetaTileEntity aMetaTileEntity =
                      ...
                      if (aMetaTileEntity == null)
                        return false;
                      if (aMetaTileEntity instanceof
                          GT_MetaTileEntity_Hatch_Output hatch) {
                        hatch.updateTexture(aBaseCasingIndex);
                        hatch.updateCraftingIcon(this.getMachineCraftingIcon());
                    ...
                    protected void setHatchRecipeMap(
                        GT_MetaTileEntity_Hatch_Input hatch) {
                      if (filtersFluid()) {
                        hatch.mRecipeMap = getRecipeMap();
                      }
                    ...
                    /**
                     * @return If this multi filters fluid input for hatches
                     *     based on recipemap.
                     */
                    protected boolean filtersFluid() { return true; }
                    ...
                    public String[] getInfoData() {
                      ...
                      for (GT_MetaTileEntity_Hatch_Muffler tHatch :
                           filterValidMTEs(mMufflerHatches)) {
                        mPollutionReduction =
                            Math.max(tHatch.calculatePollutionReduction(100),
                                     mPollutionReduction);
                      }
                      ...
                      for (GT_MetaTileEntity_Hatch_Energy tHatch :
                           filterValidMTEs(mEnergyHatches)) {
                        storedEnergy +=
                            tHatch.getBaseMetaTileEntity().getStoredEU();
                        maxEnergy +=
                            tHatch.getBaseMetaTileEntity().getEUCapacity();
                      }

                      return new String[] {
                          /* 1 */ StatCollector.translateToLocal(
                          ...
                          /* 5 */
                              StatCollector.translateToLocal(
                                  "GT5U.multiblock.problems") +
                              ": " + EnumChatFormatting.RED +
                              (getIdealStatus() - getRepairStatus()) +
                              EnumChatFormatting.RESET + " " +
                              StatCollector.translateToLocal(
                                  "GT5U.multiblock.efficiency") +
                              ": " + EnumChatFormatting.YELLOW +
                              mEfficiency / 100.0F + EnumChatFormatting.RESET +
                              " %",
                          /* 6 */
                              StatCollector.translateToLocal(
                                  "GT5U.multiblock.pollution") +
                              ": " + EnumChatFormatting.GREEN +
                    ...
                    public boolean allowPullStack(
                        IGregTechTileEntity aBaseMetaTileEntity, int aIndex,
                    ...
                    protected ItemStack[] getCompactedInputs() {
                      ...
                      for (int i = 0; i < tInputList_sS - 1; i++) {
                        for (int j = i + 1; j < tInputList_sS; j++) {
                          if (!GT_Utility.areStacksEqual(tInputList.get(i),
                                                         tInputList.get(j)))
                            continue;
                          if (tInputList.get(i).stackSize >=
                              tInputList.get(j).stackSize) {
                            tInputList.remove(j--);
                    ...
                    protected FluidStack[] getCompactedFluids() {
                      ...
                      for (int i = 0; i < tFluidList_sS - 1; i++) {
                        for (int j = i + 1; j < tFluidList_sS; j++) {
                          ...
                          if (tFluidList.get(i).amount >=
                              tFluidList.get(j).amount) {
                            tFluidList.remove(j--);
                    ...
                    public void getWailaBody(ItemStack itemStack,
                      ...
                      if (tag.getBoolean("incompleteStructure")) {
                        currentTip.add(RED + "** INCOMPLETE STRUCTURE **" +
                                       RESET);
                      }
                      currentTip.add((tag.getBoolean("hasProblems")
                                          ? (RED + "** HAS PROBLEMS **")
                                          : GREEN + "Running Fine") +
                                     RESET + "  Efficiency: " +
                          ...
                          if (actualEnergyUsage > 0) {
                            currentTip.add(
                                StatCollector.translateToLocalFormatted(
                                    ...
                                    GT_Utility.getAmperageForTier(
                                        actualEnergyUsage, (byte)energyTier),
                                    GT_Utility.getColoredTierNameFromTier(
                                        (byte)energyTier)));
                          ...
                          if (actualEnergyUsage > 0) {
                            currentTip.add(
                                StatCollector.translateToLocalFormatted(
                                    ...
                                    formatNumbers(actualEnergyUsage),
                                    GT_Utility.getColoredTierNameFromVoltage(
                                        actualEnergyUsage)));
                          } else if (actualEnergyUsage < 0) {
                            currentTip.add(
                                StatCollector.translateToLocalFormatted(
                      ...
                      currentTip.add(GT_Waila.getMachineProgressString(
                          isActive, tag.getInteger("maxProgress"),
                          tag.getInteger("progress")));
                      // Show ns on the tooltip
                      if (GT_Mod.gregtechproxy.wailaAverageNS &&
                          tag.hasKey("averageNS")) {
                        int tAverageTime = tag.getInteger("averageNS");
                        currentTip.add("Average CPU load of ~" +
                                       formatNumbers(tAverageTime) + " ns");
                      }
                      super.getWailaBody(itemStack, currentTip, accessor,
                                         config);
                    }

                    @Override
                    public void getWailaNBTData(
                        EntityPlayerMP player, TileEntity tile,
                        NBTTagCompound tag, World world, int x, int y, int z) {
                      super.getWailaNBTData(player, tile, tag, world, x, y, z);

                      tag.setBoolean("hasProblems", (getIdealStatus() -
                                                     getRepairStatus()) > 0);
                      ...
                      tag.setBoolean(
                          "incompleteStructure",
                          (getBaseMetaTileEntity().getErrorDisplayID() & 64) !=
                              0);

                      final IGregTechTileEntity tileEntity =
                          getBaseMetaTileEntity();
                      if (tileEntity != null) {
                        tag.setBoolean("isActive", tileEntity.isActive());
                        if (tileEntity.isActive()) {
                          if (mEUt < 0)
                            tag.setLong("energyUsage", getActualEnergyUsage());
                          else
                            tag.setLong("energyUsage",
                                        (long)-mEUt * mEfficiency / 10000);
                          tag.setLong("energyTier", getInputVoltageTier());
                        }
                      }

                      final GT_ClientPreference preference =
                          GT_Mod.gregtechproxy.getClientPreference(
                              player.getUniqueID());
                      if (preference != null &&
                          preference.isWailaAverageNSEnabled()) {
                        getBaseMetaTileEntity().startTimeStatistics();
                        int tAverageTime = 0;
                        int amountOfZero = 0;
                        for (int tTime :
                             this.getBaseMetaTileEntity().getTimeStatistics()) {
                          tAverageTime += tTime;
                          if (tTime == 0) {
                            amountOfZero += 1;
                          }
                        ...
                        int samples =
                            getBaseMetaTileEntity().getTimeStatistics().length -
                            amountOfZero;
                        if (samples > 0) {
                    ...
                    protected void setMufflers(boolean state) {
                      for (GT_MetaTileEntity_Hatch_Muffler aMuffler :
                           mMufflerHatches) {
                        final IGregTechTileEntity iGTTileEntity =
                            aMuffler.getBaseMetaTileEntity();
                    ...
                    public List<GT_MetaTileEntity_Hatch>
                    getExoticEnergyHatches() {
                      return mExoticEnergyHatches;
                    }

                    /**
                     * @return Returns true if there is 1 TT Energy Hatch OR up
                     *     to 2 Energy Hatches
                     */
                    ...
                    /**
                     * Checks if all the item / fluid outputs of the recipe can
                     * be outputted to the buses / hatches. If void protection
                     * is enabled, it also checks for {@link
                     * #protectsExcessItem()} and
                     * {@link #protectsExcessFluid()}, so you don't need to call
                     * them along with this method. <p> If you're using {@link
                     * GT_ParallelHelper}, it will handle void protection and
                     * return 0 parallel if all the output cannot be dumped into
                     * buses / hatches. In that case you won't use this method.
                     */
                    protected boolean canOutputAll(@Nonnull GT_Recipe recipe) {
                      return canOutputAll(recipe.mOutputs,
                                          recipe.mFluidOutputs);
                    }

                    /**
                     * Checks if all the items can be outputted to the output
                     * buses. If void protection is enabled, it also checks for
                     * {@link #protectsExcessItem()}, so you don't need to call
                     * it along with this method.
                     */
                    ...
                    /**
                     * Checks if all the fluids can be outputted to the output
                     * hatches. If void protection is enabled, it also checks
                     * for {@link #protectsExcessFluid()}, so you don't need to
                     * call it along with this method.
                     */
                    ...
                    /**
                     * Checks if all the items / fluids can be outputted to
                     * output buses / hatches. If void protection is enabled, it
                     * also checks for {@link #protectsExcessItem()} and
                     * {@link #protectsExcessFluid()}, so you don't need to call
                     * them along with this method.
                     */
                    protected boolean canOutputAll(
                        @Nullable ItemStack[] items,
                        @Nullable FluidStack[] fluids) {
                      if (!protectsExcessItem() && !protectsExcessFluid()) {
                        return true;
                      }

                      VoidProtectionHelper voidProtectionHelper =
                          new VoidProtectionHelper()
                              .setMachine(this)
                    ...
                    public boolean isAllowedToWork() {
                      return getBaseMetaTileEntity() != null &&
                    ...
                    @Override
                    public List<ItemStack> getItemOutputSlots(
                        ItemStack[] toOutput) {
                      List<ItemStack> ret = new ArrayList<>();
                      for (final GT_MetaTileEntity_Hatch tBus :
                           filterValidMTEs(mOutputBusses)) {
                    ...
                    @Override
                    public List<? extends IFluidStore> getFluidOutputSlots(
                        FluidStack[] toOutput) {
                      return filterValidMTEs(mOutputHatches);
                    }

                    /**
                     * Util method for DT-like structure to collect list of
                     * output hatches.
                     */
                    protected <T extends GT_MetaTileEntity_Hatch_Output>
                        List<? extends IFluidStore> getFluidOutputSlotsByLayer(
                            FluidStack[] toOutput,
                      ...
                      for (int i = 0; i < toOutput.length; i++) {
                        ...
                        for (GT_MetaTileEntity_Hatch_Output hatch :
                             hatchesByLayer.get(i)) {
                          if (!hatch.isValid())
                            continue;
                          if (fluidOutputForLayer != null) {
                            ret.add(new OutputHatchWrapper(
                                hatch,
                                f
                                -> GT_Utility.areFluidsEqual(
                                    f, fluidOutputForLayer)));
                          } else {
                            ret.add(hatch);
                          }
                        }
                    ...
                    public boolean canDumpItemToME() {
                      for (GT_MetaTileEntity_Hatch tHatch :
                           filterValidMTEs(mOutputBusses)) {
                        if (tHatch instanceof
                            GT_MetaTileEntity_Hatch_OutputBus_ME) {
                          return true;
                        }
                      }
                      return false;
                    ...
                    public boolean canDumpFluidToME() {
                      for (IFluidStore tHatch :
                           getFluidOutputSlots(new FluidStack[0])) {
                        if (tHatch instanceof
                    ...
                    @Override
                    public void setSingleRecipeCheck(
                        SingleRecipeCheck recipeCheck) {
                    ...
                    public void addUIWidgets(ModularWindow.Builder builder,
                      ...
                      builder.widget(
                          inventorySlot.setPos(173, 167).setBackground(
                              GT_UITextures.SLOT_DARK_GRAY));

                      final DynamicPositionedColumn screenElements =
                          new DynamicPositionedColumn();
                    ...
                    protected String generateCurrentRecipeInfoString() {
                      StringBuffer ret = new StringBuffer(
                          EnumChatFormatting.WHITE + "Progress: ");
                      ...
                      numberFormat.format(
                          (double)mProgresstime / mMaxProgresstime * 100, ret);
                      ret.append("%)\n");
                      numberFormat.setMinimumFractionDigits(0);
                      numberFormat.setMaximumFractionDigits(2);

                      IntConsumer appendRate = (amount) -> {
                        double processPerTick =
                            (double)amount / mMaxProgresstime * 20;
                        if (processPerTick > 1) {
                          ret.append(" (");
                          numberFormat.format(
                              Math.round(processPerTick * 10) / 10.0, ret);
                          ret.append("/s)");
                        } else {
                          ret.append(" (");
                          numberFormat.format(
                              Math.round(1 / processPerTick * 10) / 10.0, ret);
                          ret.append("s/ea)");
                        }
                      };
                    ...
                    protected void drawTexts(
                        DynamicPositionedColumn screenElements,
                        SlotWidget inventorySlot) {
                      screenElements.setSynced(false).setSpace(0).setPos(10, 7);

                      screenElements
                          .widget(new TextWidget(
                                      GT_Utility.trans("132", "Pipe is loose."))
                                      .setDefaultColor(COLOR_TEXT_WHITE.get())
                                      .setEnabled(widget -> !mWrench))
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              () -> mWrench, val -> mWrench = val));
                      screenElements
                          .widget(
                              new TextWidget(
                                  GT_Utility.trans("133", "Screws are loose."))
                                  .setDefaultColor(COLOR_TEXT_WHITE.get())
                                  .setEnabled(widget -> !mScrewdriver))
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              () -> mScrewdriver, val -> mScrewdriver = val));
                      screenElements
                          .widget(
                              new TextWidget(GT_Utility.trans(
                                                 "134", "Something is stuck."))
                                  .setDefaultColor(COLOR_TEXT_WHITE.get())
                                  .setEnabled(widget -> !mSoftHammer))
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              () -> mSoftHammer, val -> mSoftHammer = val));
                      screenElements
                          .widget(
                              new TextWidget(GT_Utility.trans(
                                                 "135", "Platings are dented."))
                                  .setDefaultColor(COLOR_TEXT_WHITE.get())
                                  .setEnabled(widget -> !mHardHammer))
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              () -> mHardHammer, val -> mHardHammer = val));
                      screenElements
                          .widget(new TextWidget(
                                      GT_Utility.trans("136",
                          ...
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              ()
                                  -> mSolderingTool,
                              val -> mSolderingTool = val));
                      screenElements
                          .widget(new TextWidget(
                                      GT_Utility.trans(
                                          "137", "That doesn't belong there."))
                                      .setDefaultColor(COLOR_TEXT_WHITE.get())
                                      .setEnabled(widget -> !mCrowbar))
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              () -> mCrowbar, val -> mCrowbar = val));
                      screenElements
                          .widget(new TextWidget(
                                      GT_Utility.trans("138",
                                                       "Incomplete Structure."))
                                      .setDefaultColor(COLOR_TEXT_WHITE.get())
                                      .setEnabled(widget -> !mMachine))
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              () -> mMachine, val -> mMachine = val));
                      screenElements.widget(
                          new TextWidget("Too Uncertain.")
                              .setDefaultColor(COLOR_TEXT_WHITE.get())
                              .setEnabled(widget
                                          -> (getBaseMetaTileEntity()
                                                  .getErrorDisplayID() &
                                              128) != 0));
                      screenElements.widget(
                          new TextWidget("Invalid Parameters.")
                              .setDefaultColor(COLOR_TEXT_WHITE.get())
                              .setEnabled(widget
                                          -> (getBaseMetaTileEntity()
                                                  .getErrorDisplayID() &
                          ...
                          .widget(new ShutDownReasonSyncer(
                              ()
                                  -> getBaseMetaTileEntity()
                                         .getLastShutDownReason(),
                              reason
                              -> getBaseMetaTileEntity().setShutDownReason(
                                  reason)))
                          .widget(new FakeSyncWidget.BooleanSyncer(
                              ()
                                  -> getBaseMetaTileEntity().wasShutdown(),
                              wasShutDown
                              -> getBaseMetaTileEntity().setShutdownStatus(
                                  wasShutDown)));
                      ...
                      if (showRecipeTextInGUI()) {
                        ...
                        screenElements
                            .widget(
                                TextWidget
                                    .dynamicString(
                                        this::generateCurrentRecipeInfoString)
                                    .setSynced(false)
                                    .setTextAlignment(Alignment.CenterLeft)
                                    .setEnabled(
                                        widget
                                        -> (mOutputFluids != null &&
                                            mOutputFluids.length > 0) ||
                                               (mOutputItems != null &&
                                                mOutputItems.length > 0)))
                            .widget(new FakeSyncWidget.ListSyncer<>(
                                ()
                                    -> mOutputFluids != null
                                           ? Arrays.asList(mOutputFluids)
                                           : Collections.emptyList(),
                                val
                                -> mOutputFluids =
                                       val.toArray(new FluidStack[0]),
                                NetworkUtils::writeFluidStack,
                                NetworkUtils::readFluidStack))
                            .widget(new FakeSyncWidget.ListSyncer<>(
                                ()
                                    -> mOutputItems != null
                                           ? Arrays.asList(mOutputItems)
                                           : Collections.emptyList(),
                                val
                            ...
                            .widget(new FakeSyncWidget.IntegerSyncer(
                                ()
                                    -> mProgresstime,
                                val -> mProgresstime = val))
                            .widget(new FakeSyncWidget.IntegerSyncer(
                                ()
                                    -> mMaxProgresstime,
                                val -> mMaxProgresstime = val));
                      }
                          ...
                          new TextWidget(
                              ...
                              .setEnabled(widget -> {
                                ...
                                if (getBaseMetaTileEntity()
                                  ...
                                  final ItemStack tItem =
                                      inventorySlot.getMcSlot().getStack();
                                  return tItem == null ||
                                      !(tItem.getItem() ==
                    ...
                    protected boolean showRecipeTextInGUI() { return true; }

                    @TestOnly
                    protected void setEnergyHatches(
                        ArrayList<GT_MetaTileEntity_Hatch_Energy>
                            EnergyHatches) {
                      this.mEnergyHatches = EnergyHatches;
                    }

                    @TestOnly
                    protected void setExoticEnergyHatches(
                        List<GT_MetaTileEntity_Hatch> ExoticEnergyHatches) {
