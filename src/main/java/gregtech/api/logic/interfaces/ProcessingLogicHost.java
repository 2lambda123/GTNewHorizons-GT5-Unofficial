...
public interface ProcessingLogicHost<P extends MuTEProcessingLogic<P>>
   ...
   */
  @Nonnull P getProcessingLogic();

  boolean isInputSeparated();

  void setInputSeparation(Boolean inputSeparation);

  default boolean supportsInputSeparation() { return true; }

  default boolean getDefaultInputSeparationMode() { return false; }

  boolean isRecipeLockingEnabled();

  void setRecipeLocking(Boolean recipeLocked);

  default boolean supportsSingleRecipeLocking() { return true; }

  default boolean getDefaultRecipeLockingMode() { return false; }

  default boolean supportsBatchMode() { return true; }

  void setBatchMode(Boolean batchMode);

  boolean isBatchModeEnabled();

  default boolean getDefaultBatchMode() { return false; }

  /**
   * Get what the machine can void or not
   */
  @Nonnull VoidingMode getVoidMode();

  /**
   * Called when the processing logic should be updated by {@link
   * #needsUpdate()}
   */
   ...
   *
   * If you want to make the processing logic be updated call {@link
   * #setProcessingUpdate(boolean)}
   */
  boolean needsUpdate();

  /**
   * To be called when one needs to updated the processing logic. That can be
   * when parallel changes, ect.
   */
