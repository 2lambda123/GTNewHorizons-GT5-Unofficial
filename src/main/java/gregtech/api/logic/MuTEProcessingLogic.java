...
public class MuTEProcessingLogic<P extends MuTEProcessingLogic<P>>
  ...
  @Nullable protected UUID itemOutputID;
  @Nullable protected UUID fluidOutputID;
  ...
  public CheckRecipeResult process() {
    return CheckRecipeResultRegistry
        .NO_RECIPE; /*
                     ...
                     * for (Map.Entry<UUID, ItemInventoryLogic> itemEntry :
                     * machineHost
                     * .getAllItemInventoryLogics(InventoryType.Input)) {
                     * itemOutputID =
                     * Objects.requireNonNull(itemEntry.getKey()); itemInput =
                     * Objects.requireNonNull(itemEntry.getValue()); fluidInput
  ...
  processRecipe(@Nonnull List<GT_Recipe> recipes,
                @Nonnull ItemInventoryLogic itemInput,
    ...
    for (GT_Recipe recipe : recipes) {
      ...
      GT_ParallelHelper helper =
          createParallelHelper(recipe, itemInput, fluidInput);
   ...
   */
  @Nonnull
  protected Object findRecipe(@Nullable RecipeMap<?> map,
                              @Nonnull ItemInventoryLogic itemInput,
                              @Nonnull FluidInventoryLogic fluidInput) {
    if (map == null) {
      return false;
  ...
  @Nonnull
  protected GT_ParallelHelper
  createParallelHelper(@Nonnull GT_Recipe recipe,
                       @Nonnull ItemInventoryLogic itemInput,
                       @Nonnull FluidInventoryLogic fluidInput) {
  ...
  public void startCheck() { recipeResult = process(); }
  ...
  protected void output() {
    ItemInventoryLogic itemOutput =
        machineHost.getItemLogic(InventoryType.Output, itemOutputID);
    FluidInventoryLogic fluidOutput =
        machineHost.getFluidLogic(InventoryType.Output, fluidOutputID);
  ...
  public void loadFromNBT(@Nonnull NBTTagCompound logicNBT) {
    ...
    if (logicNBT.hasKey("itemOutputs")) {
      NBTTagList itemOutputsNBT =
          logicNBT.getTagList("itemOutputs", TAG_COMPOUND);
      outputItems = new ItemStack[itemOutputsNBT.tagCount()];
      for (int i = 0; i < itemOutputsNBT.tagCount(); i++) {
        outputItems[i] =
            GT_Utility.loadItem(itemOutputsNBT.getCompoundTagAt(i));
      }
    }
    if (logicNBT.hasKey("fluidOutputs")) {
      NBTTagList fluidOutputsNBT =
          logicNBT.getTagList("fluidOutputs", TAG_COMPOUND);
      outputFluids = new FluidStack[fluidOutputsNBT.tagCount()];
      for (int i = 0; i < fluidOutputsNBT.tagCount(); i++) {
        outputFluids[i] = FluidStack.loadFluidStackFromNBT(
            fluidOutputsNBT.getCompoundTagAt(i));
      }
    }
  ...
  /**
   * Returns a gui part, which will be displayed in a separate tab on the
   * machine's gui.
   */
  @Nonnull
