...
public class PollutionTask<T extends TaskHost & MachineProgress>
    extends TickableTask<T> {

  private int pollutionPerSecond;
  private static final int POLLUTION_TICK = TickTime.SECOND;

  public PollutionTask(@Nonnull T taskHost) { super(taskHost); }
  ...
  public int getPollutionPerSecond() { return pollutionPerSecond; }
  ...
  public void update(long tick, boolean isServerSide) {
    if (isServerSide && tick % POLLUTION_TICK == 0 &&
        taskHost.hasThingsToDo()) {
