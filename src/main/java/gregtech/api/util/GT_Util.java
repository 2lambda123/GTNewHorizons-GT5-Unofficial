...
public class GT_Util {
  ...
  public static final ThreadLocal<TileEntity> LAST_BROKEN_TILEENTITY =
      new ThreadLocal<>();
  ...
  public static NBTTagCompound fuseNBT(NBTTagCompound aNBT1,
                                       NBTTagCompound aNBT2) {
    if (aNBT1 == null)
      return aNBT2 == null ? new NBTTagCompound()
                           : (NBTTagCompound)aNBT2.copy();
    final NBTTagCompound rNBT = (NBTTagCompound)aNBT1.copy();
    if (aNBT2 == null)
      return rNBT;
    for (Object tKey : aNBT2.func_150296_c() /* getKeySet */)
      if (!rNBT.hasKey(tKey.toString()))
        rNBT.setTag(tKey.toString(), aNBT2.getTag(tKey.toString()));
    return rNBT;
  }

  /**
   * Construct a NBTTagCompound from a series of key, value pairs. Inspired from
   * GT6.
   */
   ...
   * Get a TileEntity
   */
  public static TileEntity getTileEntity(World aWorld, int aX, int aY, int aZ,
    ...
    if (aLoadUnloadedChunks || aWorld.blockExists(aX, aY, aZ)) {
      ...
      if (rTileEntity instanceof IMultiTileEntity && rTileEntity.isInvalid())
        return null;
      if (rTileEntity != null)
        return rTileEntity;
      rTileEntity = LAST_BROKEN_TILEENTITY.get();
      if (rTileEntity != null && rTileEntity.xCoord == aX &&
          rTileEntity.yCoord == aY && rTileEntity.zCoord == aZ)
        return rTileEntity;
    }
    return null;
  }
  ...
  /**
   * Sets the TileEntity at the passed position, with the option of turning
   * adjacent TileEntity updates off.
   */
  public static TileEntity setTileEntity(World aWorld, int aX, int aY, int aZ,
    ...
    if (aCauseTileEntityUpdates)
      ...
      if (tChunk != null) {
        ...
        tChunk.func_150812_a /* setBlockTileEntityInChunk */ (
            aX & 15, aY, aZ & 15, aTileEntity);
  ...
  public static boolean setTileEntity(World aWorld, int aX, int aY, int aZ,
                                      Block aBlock, short aMeta, long aFlags,
  ...
  public static TileEntity getTileEntity(World aWorld, ChunkCoordinates aCoords,
                                         boolean aLoadUnloadedChunks) {
    return getTileEntity(aWorld, aCoords.posX, aCoords.posY, aCoords.posZ,
                         aLoadUnloadedChunks);
  }
  ...
  public static boolean markChunkDirty(World aWorld, int aX, int aZ) {
    ...
    if (aChunk == null) {
      ...
      if (aChunk == null) {
        GT_Log.err.println(
            "Some important Chunk does not exist for some reason at Coordinates X: " +
            aX + " and Z: " + aZ);
        return false;
      }
    }
  ...
  public static boolean markChunkDirty(Object aTileEntity) {
    return aTileEntity instanceof TileEntity &&
        markChunkDirty(((TileEntity)aTileEntity).getWorldObj(),
                       ((TileEntity)aTileEntity).xCoord,
                       ((TileEntity)aTileEntity).zCoord);
  }

  public static int mixRGBInt(int aRGB1, int aRGB2) {
    return getRGBInt(new short[] {(short)((getR(aRGB1) + getR(aRGB2)) >> 1),
  ...
  public static int getRGBInt(short[] aColors) {
    return aColors == null
        ? 16777215
        : (aColors[0] << 16) | (aColors[1] << 8) | aColors[2];
  }

  public static int getRGBaInt(short[] aColors) {
    return aColors == null ? 16777215
                           : (aColors[0]) << 16 | (aColors[1] << 8) |
                                 aColors[2] | (aColors[3] << 24);
  }

  public static String toHexString(short[] aColors) {
    return aColors == null
        ? "FFFFFF"
        : Integer.toHexString((aColors[0] << 16) | (aColors[1] << 8) |
                              aColors[2]);
  }
  ...
  public static short[] getRGBaArray(int aColors) {
    return new short[] {(short)((aColors >>> 16) & 255),
                        (short)((aColors >>> 8) & 255), (short)(aColors & 255),
