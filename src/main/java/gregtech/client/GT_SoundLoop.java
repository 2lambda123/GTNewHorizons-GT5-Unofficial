...
public class GT_SoundLoop extends MovingSound {
  ...
  public GT_SoundLoop(ResourceLocation p_i45104_1_, IGregTechTileEntity base,
                      boolean stopWhenActive, boolean stopWhenInactive) {
    super(p_i45104_1_);
  ...
  public GT_SoundLoop(ResourceLocation sound, MultiTileBasicMachine<?> base,
                      boolean stopWhenActive, boolean stopWhenInactive) {
    super(sound);
    this.whileActive = stopWhenActive;
    this.whileInactive = stopWhenInactive;
  ...
  public void update() {
    ...
    donePlaying = world.provider.dimensionId != worldID ||
                  !world.checkChunksExist((int)xPosF, (int)yPosF, (int)zPosF,
                                          (int)xPosF, (int)yPosF, (int)zPosF);
    if (donePlaying)
      return;
    TileEntity tile = world.getTileEntity((int)xPosF, (int)yPosF, (int)zPosF);
    if ((tile instanceof IGregTechTileEntity)) {
      fadeMe |=
          ((IGregTechTileEntity)tile).isActive() ? whileActive : whileInactive;
      return;
    }

    if ((tile instanceof MultiTileBasicMachine)) {
      fadeMe |= ((MultiTileBasicMachine<?>)tile).isActive() ? whileActive
                                                            : whileInactive;
      return;
    }
