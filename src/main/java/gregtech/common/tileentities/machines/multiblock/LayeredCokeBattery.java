...
public class LayeredCokeBattery
    extends StackableModularController<LayeredCokeBattery,
                                       LayeredCokeBatteryProcessingLogic> {

  private static IStructureDefinition<LayeredCokeBattery>
      STRUCTURE_DEFINITION_MEGA = null;
  ...
  private static final Vec3Impl STRUCTURE_OFFSET_MEGA_POSITION =
      new Vec3Impl(4, 7, -4);
  private static final Vec3Impl STRUCTURE_OFFSET_MEGA_START =
      new Vec3Impl(0, 0, -3);
  private static final Vec3Impl STRUCTURE_OFFSET_MEGA_STACK =
      new Vec3Impl(0, 0, -2);
  private static final Vec3Impl STRUCTURE_OFFSET_MEGA_STOP =
      new Vec3Impl(0, 0, -1);
  ...
  public String getLocalName() { return "Layered Coke Battery"; }
  ...
  public UpgradeCasings getBaseMucType() { return UpgradeCasings.Heater; }

  public int getParallelFactor() { return 2; }
  ...
  public void construct(ItemStack trigger, boolean hintsOnly) {
    ...
    buildPiece(STRUCTURE_PIECE_BASE, trigger, hintsOnly,
               buildState.getCurrentOffset());
    buildState.addOffset(getMegaPositionOffset());

    if (stackCount >= 1) {
      buildPiece(STACKABLE_START, trigger, hintsOnly,
                 buildState.getCurrentOffset());
      buildState.addOffset(getStartingStackOffset());

      for (int i = 0; i < stackCount; i++) {
        buildPiece(STACKABLE_MIDDLE, trigger, hintsOnly,
                   buildState.getCurrentOffset());
        buildState.addOffset(getPerStackOffset());
      }
      if (hasTop()) {
        buildPiece(STACKABLE_STOP, trigger, hintsOnly,
                   buildState.getCurrentOffset());
      }
    }
  ...
  public IStructureDefinition<LayeredCokeBattery> getStructureDefinition() {
    if (STRUCTURE_DEFINITION_MEGA == null) {
      STRUCTURE_DEFINITION_MEGA =
          StructureDefinition.<LayeredCokeBattery>builder()
              .addShape(STRUCTURE_PIECE_BASE,
                        transpose(new String[][] {
                            {" AAA ", "AAAAA", "AEEEP", "AAAAA"},
                            {" AAA ", "A   A", "A   A", "AAAAA"},
                            {" A~A ", "A   A", "A   A", "AAAAA"},
                            {" AAA ", "A   A", "A   A", "AAAAA"},
                            {" AAA ", "AAAAA", "AAAAA", "AAAAA"}}))
              .addShape(STACKABLE_STOP,
                        transpose(new String[][] {
                            {"AHFFFFAFFFFHA", "AAAAAAAAAAAAA"},
                            {" B B B B B B ", "AFAFAFAFAFAFA"},
                            {"HB B B B B BH", "AFAFAFAFAFAFA"},
                            {" B B B B B B ", "AFAFAFAFAFAFA"},
                            {" B B B B B B ", "AFAFAFAFAFAFA"},
                            {" B B B B B B ", "AFAFAFAFAFAFA"},
                            {" B B B B B B ", "AFAFAFAFAFAFA"},
              ...
              .addShape(STACKABLE_MIDDLE,
                        transpose(new String[][] {
                            {"AHFFFFAFFFFHA", "AAAAAAAAAAAAA"},
                            {" B    A    B ", "AAAAAAAAAAAAA"},
                            {"HB    A    BH", "AAAAAAAAAAAAA"},
                            {" B    A    B ", "AAAAAAAAAAAAA"},
                            {" B    A    B ", "AAAAAAAAAAAAA"},
                            {" B    A    B ", "AAAAAAAAAAAAA"},
                            {" B    A    B ", "AAAAAAAAAAAAA"},
              ...
              .addShape(
                  STACKABLE_START,
                  transpose(new String[][] {
                      {"AAAAAAAAAAAAA", "AHFFFFAFFFFHA", "AAAAAAAAAAAAA"},
              ...
              .addElement('H', ofMuTECasings(NOTHING.getValue(), HEATER_CASINGS,
                                             INSULATOR_CASINGS))
              .addElement('P',
                          ofMuTECasings(NOTHING.getValue(), AMPERAGE_CASINGS))
              .build();
    }
  ...
  public boolean checkMachine() {
    ...
    buildState.startBuilding(getStartingStructureOffset());
    if (!checkPiece(STRUCTURE_PIECE_BASE, buildState.getCurrentOffset()))
  ...
  protected boolean calculateMucMultipliers() {
    ...
    if (totalHeaterCount + totalInsulatorCount < stackCount ||
        totalInsulatorCount > totalHeaterCount) {
      return false;
    }
  ...
  protected GT_Multiblock_Tooltip_Builder createTooltip() {
    ...
    tt.addMachineType("Coke Oven")
        ...
        .addSeparator()
        .beginVariableStructureBlock(7, 9, 2 + getMinStacks(),
                                     2 + getMaxStacks(), 7, 9, true)
        ...
        .addStructureInfo(
            String.format("Stackable middle stacks between %d-%d time(s).",
                          getMinStacks(), getMaxStacks()))
