...
public class AdvChemicalProcessor
    extends ComplexParallelController<AdvChemicalProcessor,
                                      AdvChemicalProcessorProcessingLogic> {

  private static IStructureDefinition<AdvChemicalProcessor>
  ...
  protected final ArrayList<HashSet<String>> processWhitelists =
      new ArrayList<>(MAX_PROCESSES);
  protected final ArrayList<ItemStackHandler>
      processWhitelistInventoryHandlers = new ArrayList<>(MAX_PROCESSES);
  protected final ArrayList<ArrayList<IFluidTank>> processFluidWhiteLists =
      new ArrayList<>(MAX_PROCESSES);
  ...
  public AdvChemicalProcessor() {
    ...
    for (int i = 0; i < MAX_PROCESSES; i++) {
      ...
      processWhitelistInventoryHandlers.add(
          new ItemStackHandler(ITEM_WHITELIST_SLOTS));
      ArrayList<IFluidTank> processFluidTanks =
          new ArrayList<>(FLUID_WHITELIST_SLOTS);
  ...
  public void readFromNBT(NBTTagCompound nbt) {
    ...
    for (int i = 0; i < MAX_PROCESSES; i++) {
      ...
      final NBTTagCompound itemList =
          processWhiteLists.getCompoundTag("items" + i);
      if (itemList != null) {
        processWhitelistInventoryHandlers.get(i).deserializeNBT(itemList);
      }
      final NBTTagList fluidList = processWhiteLists.getTagList(
          "fluids" + i, Constants.NBT.TAG_COMPOUND);
      ...
      for (int j = 0; j < fluidList.tagCount(); j++) {
        ...
        if (fluidStack != null) {
          processFluidWhiteLists.get(i).get(index).fill(fluidStack, true);
        }
      }
    }
  }
  ...
  public void writeToNBT(NBTTagCompound nbt) {
    ...
    for (int i = 0; i < MAX_PROCESSES; i++) {
      processWhiteLists.setTag(
          "items" + i, processWhitelistInventoryHandlers.get(i).serializeNBT());
      final NBTTagList fluidList = new NBTTagList();
      for (int j = 0; j < FLUID_WHITELIST_SLOTS; j++) {
        final FluidStack fluidStack =
            processFluidWhiteLists.get(i).get(j).getFluid();
        if (fluidStack != null) {
  ...
  public GT_Multiblock_Tooltip_Builder createTooltip() {
    final GT_Multiblock_Tooltip_Builder tt =
        new GT_Multiblock_Tooltip_Builder();
  ...
  public boolean checkMachine() {
    ...
    if (!checkPiece(STRUCTURE_PIECE_T1, buildState.getCurrentOffset()))
      return buildState.failBuilding();
    if (maxComplexParallels > 1) {
      buildState.addOffset(STRUCTURE_OFFSET_T2);
      if (!checkPiece(STRUCTURE_PIECE_T2, buildState.getCurrentOffset()))
        return buildState.failBuilding();
    }
    if (maxComplexParallels > 2) {
      buildState.addOffset(STRUCTURE_OFFSET_T3);
      if (!checkPiece(STRUCTURE_PIECE_T3, buildState.getCurrentOffset()))
        return buildState.failBuilding();
    }
    if (maxComplexParallels > 3) {
      buildState.addOffset(STRUCTURE_OFFSET_T4);
      if (!checkPiece(STRUCTURE_PIECE_T4, buildState.getCurrentOffset()))
        return buildState.failBuilding();
    }
    if (maxComplexParallels > 4) {
      buildState.addOffset(STRUCTURE_OFFSET_T5);
      if (!checkPiece(STRUCTURE_PIECE_T5_6, buildState.getCurrentOffset()))
        return buildState.failBuilding();
    }
    ...
    if (maxComplexParallels > 6) {
      ...
      if (!checkPiece(STRUCTURE_PIECE_T7_8, buildState.getCurrentOffset()))
        return buildState.failBuilding();
    }
  ...
  public void construct(ItemStack trigger, boolean hintsOnly) {
    buildState.startBuilding(getStartingStructureOffset());
    buildPiece(STRUCTURE_PIECE_T1, trigger, hintsOnly,
               buildState.getCurrentOffset());
    if (maxComplexParallels > 1) {
      buildState.addOffset(STRUCTURE_OFFSET_T2);
      buildPiece(STRUCTURE_PIECE_T2, trigger, hintsOnly,
                 buildState.getCurrentOffset());
    }
    if (maxComplexParallels > 2) {
      buildState.addOffset(STRUCTURE_OFFSET_T3);
      buildPiece(STRUCTURE_PIECE_T3, trigger, hintsOnly,
                 buildState.getCurrentOffset());
    }
    if (maxComplexParallels > 3) {
      buildState.addOffset(STRUCTURE_OFFSET_T4);
      buildPiece(STRUCTURE_PIECE_T4, trigger, hintsOnly,
                 buildState.getCurrentOffset());
    }
    if (maxComplexParallels > 4) {
      buildState.addOffset(STRUCTURE_OFFSET_T5);
      buildPiece(STRUCTURE_PIECE_T5_6, trigger, hintsOnly,
                 buildState.getCurrentOffset());
    }
    if (maxComplexParallels > 5) {
      buildState.addOffset(STRUCTURE_OFFSET_T6);
    ...
    if (maxComplexParallels > 6) {
      buildState.addOffset(STRUCTURE_OFFSET_T7);
      buildPiece(STRUCTURE_PIECE_T7_8, trigger, hintsOnly,
                 buildState.getCurrentOffset());
    }
    if (maxComplexParallels > 7) {
      buildState.addOffset(STRUCTURE_OFFSET_T8);
  ...
  public IStructureDefinition<AdvChemicalProcessor> getStructureDefinition() {
    if (STRUCTURE_DEFINITION == null) {
      STRUCTURE_DEFINITION =
          StructureDefinition.<AdvChemicalProcessor>builder()
              .addShape(STRUCTURE_PIECE_T1,
                        transpose(new String[][] {{"CPCPC", "CCCCC", "CPCPC"},
                                                  {"CGC~C", "GWWWU", "CGCCC"},
                                                  {"CPCPC", "CTTTC", "CPCPC"}}))
              .addShape(
                  STRUCTURE_PIECE_T2,
              ...
              .addElement(
                  'C', ofMuTECasings(
                           FLUID_INPUT.getValue() | ITEM_INPUT.getValue() |
                               ...
                               ITEM_OUTPUT.getValue() | ENERGY_INPUT.getValue(),
              ...
              .addElement('W', GT_StructureUtility.ofCoil(
                                   AdvChemicalProcessor::setCoilTier,
                                   AdvChemicalProcessor::getCoilTier))
  ...
  protected MultiChildWidget createMainPage(IWidgetBuilder<?> builder) {
      ...
      child.addChild(
          new ButtonWidget()
              ...
              .setOnClick((clickData, widget) -> {
                if (!widget.isClient())
                  widget.getContext().openSyncedWindow(PROCESS_WINDOW_BASE_ID +
                                                       processIndex);
              })
    ...
    child.addChild(
        new TextFieldWidget()
            .setGetterInt(() -> maxComplexParallels)
  ...
  protected ModularWindow createProcessConfigWindow(final EntityPlayer player,
                                                    final int processIndex) {
    wasWhitelistOpened = true;
    ModularWindow.Builder builder = ModularWindow.builder(86, 100);
    builder.widget(new TextWidget("Process " + processIndex)
                       .setTextAlignment(Alignment.Center)
                       .setPos(13, 7));
    builder.setBackground(GT_UITextures.BACKGROUND_SINGLEBLOCK_DEFAULT);
    builder.widget(
        SlotGroup
            .ofItemHandler(processWhitelistInventoryHandlers.get(processIndex),
                           4)
            .startFromSlot(0)
  ...
  public String getLocalName() { return "Advanced Chemical Processor"; }

  public void setCoilTier(HeatingCoilLevel coilTier) {
    this.coilTier = coilTier;
  }

  public HeatingCoilLevel getCoilTier() { return coilTier; }
  ...
  protected void generateWhitelist(int processIndex) {
    ...
    for (ItemStack itemStack :
         processWhitelistInventoryHandlers.get(processIndex).getStacks()) {
      if (itemStack != null) {
