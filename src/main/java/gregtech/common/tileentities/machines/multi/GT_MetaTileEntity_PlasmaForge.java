...
public class GT_MetaTileEntity_PlasmaForge
    extends GT_MetaTileEntity_ExtendedPowerMultiBlockBase<
        GT_MetaTileEntity_PlasmaForge> implements ISurvivalConstructable {
  ...
  // Custom long EU per tick value given that mEUt is an int. Required to
  // overclock beyond MAX voltage.
  ...
  private static final IStructureDefinition<GT_MetaTileEntity_PlasmaForge>
      STRUCTURE_DEFINITION =
          StructureDefinition.<GT_MetaTileEntity_PlasmaForge>builder()
              .addShape(STRUCTURE_PIECE_MAIN, structure_string)
              .addElement('C',
                          ofCoil(GT_MetaTileEntity_PlasmaForge::setCoilLevel,
              ...
              .addElement('b',
                          buildHatchAdder(GT_MetaTileEntity_PlasmaForge.class)
                              .atLeast(InputHatch, OutputHatch, InputBus,
                                       OutputBus, Energy, ExoticEnergy,
                                       Maintenance)
                              .casingIndex(DIM_INJECTION_CASING)
                              .dot(3)
                              .buildAndChain(GregTech_API.sBlockCasings1,
                                             DIM_INJECTION_CASING))
              .addElement(
                  'N', ofBlock(GregTech_API.sBlockCasings1, DIM_TRANS_CASING))
              .addElement(
                  's', ofBlock(GregTech_API.sBlockCasings1, DIM_BRIDGE_CASING))
              .build();

  public GT_MetaTileEntity_PlasmaForge(int aID, String aName,
                                       String aNameRegional) {
    super(aID, aName, aNameRegional);
  }

  public GT_MetaTileEntity_PlasmaForge(String aName) { super(aName); }
  ...
  protected GT_Multiblock_Tooltip_Builder createTooltip() {
    ...
    tt.addInfo("Transcending Dimensional Boundaries.")
        .addInfo("Takes " + EnumChatFormatting.RED +
                 GT_Utility.formatNumbers(max_efficiency_time_in_ticks /
                                          (3600 * 20)) +
        ...
        .addInfo(
            "When no recipe is running, fuel discount gradually decreases to " +
            EnumChatFormatting.RED + "0%" + EnumChatFormatting.GRAY + " in " +
            EnumChatFormatting.RED +
            GT_Utility.formatNumbers(efficiency_decay_time_in_ticks /
                                     (20 * 60)) +
            EnumChatFormatting.GRAY + " minutes.")
        .addInfo(AuthorColen)
        .addSeparator()
        .beginStructureBlock(33, 24, 33, false)
        .addStructureInfo(
            "DTPF Structure is too complex! See schematic for details.")
        .addStructureInfo(EnumChatFormatting.GOLD + "2,112" +
                          EnumChatFormatting.GRAY + " Heating coils required.")
        .addStructureInfo(EnumChatFormatting.GOLD + "120" +
                          EnumChatFormatting.GRAY +
                          " Dimensional bridge blocks required.")
        .addStructureInfo(EnumChatFormatting.GOLD + "1,270" +
                          EnumChatFormatting.GRAY +
                          " Dimensional injection casings required.")
        .addStructureInfo(EnumChatFormatting.GOLD + "2,121" +
                          EnumChatFormatting.GRAY +
                          " Dimensionally transcendent casings required.")
        .addStructureInfo("--------------------------------------------")
        .addStructureInfo("Requires " + EnumChatFormatting.GOLD + "1" +
                          EnumChatFormatting.GRAY + "-" +
                          EnumChatFormatting.GOLD + "2" +
                          EnumChatFormatting.GRAY + " energy hatches or " +
                          EnumChatFormatting.GOLD + "1" +
                          EnumChatFormatting.GRAY + " TT energy hatch.")
        .addStructureInfo("Requires " + EnumChatFormatting.GOLD + "1" +
                          EnumChatFormatting.GRAY + " maintenance hatch.")
        .addStructureInfo("Requires " + EnumChatFormatting.GOLD +
                          min_input_hatch + EnumChatFormatting.GRAY + "-" +
                          EnumChatFormatting.GOLD + max_input_hatch +
                          EnumChatFormatting.GRAY + " input hatches.")
        .addStructureInfo("Requires " + EnumChatFormatting.GOLD +
                          min_output_hatch + EnumChatFormatting.GRAY + "-" +
                          EnumChatFormatting.GOLD + max_output_hatch +
                          EnumChatFormatting.GRAY + " output hatches.")
        .addStructureInfo("Requires " + EnumChatFormatting.GOLD +
                          min_input_bus + EnumChatFormatting.GRAY + "-" +
                          EnumChatFormatting.GOLD + max_input_bus +
                          EnumChatFormatting.GRAY + " input busses.")
        .addStructureInfo("Requires " + EnumChatFormatting.GOLD +
                          min_output_bus + EnumChatFormatting.GRAY + "-" +
                          EnumChatFormatting.GOLD + max_input_bus +
  ...
  public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity,
                               ForgeDirection side, ForgeDirection aFacing,
                               int colorIndex, boolean aActive,
  ...
  @Override
  public IStructureDefinition<GT_MetaTileEntity_PlasmaForge>
  ...
  protected ProcessingLogic createProcessingLogic() {
    return new ProcessingLogic() {
      @Nonnull
      @Override
      protected GT_OverclockCalculator createOverclockCalculator(
          @Nonnull GT_Recipe recipe) {
      ...
      @NotNull
      @Override
      protected GT_ParallelHelper createParallelHelper(
          @Nonnull GT_Recipe recipe) {
        return super.createParallelHelper(recipeAfterDiscount(recipe));
      }

      @Override
      protected @Nonnull CheckRecipeResult validateRecipe(
          @Nonnull GT_Recipe recipe) {
      ...
      for (FluidStack fuel : valid_fuels) {
        if (tRecipe.mFluidInputs[i].isFluidEqual(fuel)) {
          ...
          tRecipe.mFluidInputs[i].amount =
              (int)Math.round(tRecipe.mFluidInputs[i].amount * discount);
          break outside;
        }
      }
    }
  ...
  public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity,
    ...
    // If there are no energy hatches or TT energy hatches, structure will fail
    // to form.
  ...
  public String[] getInfoData() {
    ...
    for (GT_MetaTileEntity_Hatch tHatch :
         filterValidMTEs(mExoticEnergyHatches)) {
    ...
    // Calculate discount to make sure it is shown properly even when machine is
    // off but decaying
    if (idle_time >= 0) {
      recalculateDiscount();
    }
    ...
    return new String[] {
        ...
        StatCollector.translateToLocal("GT5U.multiblock.Progress") + ": " +
            EnumChatFormatting.GREEN + GT_Utility.formatNumbers(mProgresstime) +
            EnumChatFormatting.RESET + "t / " + EnumChatFormatting.YELLOW +
            GT_Utility.formatNumbers(mMaxProgresstime) +
            EnumChatFormatting.RESET + "t",
        ...
        StatCollector.translateToLocal("GT5U.multiblock.mei") + ": " +
            EnumChatFormatting.YELLOW + GT_Utility.formatNumbers(voltage) +
            EnumChatFormatting.RESET + " EU/t(*" + EnumChatFormatting.YELLOW +
            amps + EnumChatFormatting.RESET + "A) " +
            StatCollector.translateToLocal("GT5U.machines.tier") + ": " +
            EnumChatFormatting.YELLOW + VN[GT_Utility.getTier(voltage)] +
        ...
        "Ticks run: " + EnumChatFormatting.GREEN +
            GT_Utility.formatNumbers(running_time) + EnumChatFormatting.RESET +
            ", Fuel Discount: " + EnumChatFormatting.RED +
  ...
  private void recalculateDiscount() {
    ...
    if (idle_time == 0 && running_time > 0) {
      ...
      // 'recover' from previous decay by setting the running time
      // appropriately. Otherwise, the fuel efficiency will just start counting
      // from zero again. should note that a lower discount number is 'better',
      // hence this seemingly inverted check.
      if (discount < proposedDiscount) {
        // Adjust running time for current discount (and add this recipe's
        // progress to it)
        double adjustedTimePercentage = 2.0 * (1.0 - discount);
        running_time =
            Math.round(adjustedTimePercentage * max_efficiency_time_in_ticks) +
            mMaxProgresstime;
      ...
      // If we had any idle time before this, use that time to set the discount
      // instead
  ...
  public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
    if (aBaseMetaTileEntity.isServerSide() &&
      ...
      isMultiChunkloaded = false;
               ...
               aBaseMetaTileEntity.isAllowedToWork() && !isMultiChunkloaded) {
      ...
      GT_ChunkManager.requestChunkLoad(
          (TileEntity)aBaseMetaTileEntity,
          new ChunkCoordIntPair(ControllerXCoordinate + 16,
                                ControllerZCoordinate));
      GT_ChunkManager.requestChunkLoad(
          (TileEntity)aBaseMetaTileEntity,
          new ChunkCoordIntPair(ControllerXCoordinate - 16,
                                ControllerZCoordinate));
      GT_ChunkManager.requestChunkLoad(
          (TileEntity)aBaseMetaTileEntity,
          new ChunkCoordIntPair(ControllerXCoordinate,
                                ControllerZCoordinate + 16));
      GT_ChunkManager.requestChunkLoad(
          (TileEntity)aBaseMetaTileEntity,
          new ChunkCoordIntPair(ControllerXCoordinate,
                                ControllerZCoordinate - 16));
  ...
  public int survivalConstruct(ItemStack stackSize, int elementBudget,
    ...
    return survivialBuildPiece(STRUCTURE_PIECE_MAIN, stackSize, 16, 21, 16,
                               realBudget, env, false, true);
  }
