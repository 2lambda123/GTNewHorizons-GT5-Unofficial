 ...
 */
public class MachineGUIProvider<
    T extends GUIHost & ProcessingLogicHost<? extends MuTEProcessingLogic<?>> &
                  PowerLogicHost & MachineProgress> extends GUIProvider<T> {

  private static final int LOGO_SIZE = 17;
  @Nonnull
  protected static final Pos2d POWER_SWITCH_BUTTON_DEFAULT_POS =
      new Pos2d(144, 0);
  @Nonnull
  protected static final Pos2d VOIDING_MODE_BUTTON_DEFAULT_POS =
      new Pos2d(54, 0);
  @Nonnull
  protected static final Pos2d INPUT_SEPARATION_BUTTON_DEFAULT_POS =
      new Pos2d(36, 0);
  @Nonnull
  protected static final Pos2d BATCH_MODE_BUTTON_DEFAULT_POS = new Pos2d(18, 0);
  @Nonnull
  protected static final Pos2d RECIPE_LOCKING_BUTTON_DEFAULT_POS =
      new Pos2d(0, 0);

  public MachineGUIProvider(@Nonnull T host) { super(host); }

  @Override
  protected void attachSynchHandlers(@Nonnull Builder builder,
                                     @Nonnull UIBuildContext uiContext) {}

  @Override
  protected void addWidgets(@Nonnull Builder builder,
                            @Nonnull UIBuildContext uiContext) {
    int page = 0;
    ...
    tabs.addTabButton(new TabButton(page++)
                          .setBackground(false,
                                         ModularUITextures.VANILLA_TAB_TOP_START
                                             .getSubArea(0, 0, 1f, 0.5f),
                                         new ItemDrawable(host.getAsItem())
                                             .withFixedSize(16, 16)
                                             .withOffset(2, 4))
                          .setBackground(true,
                                         ModularUITextures.VANILLA_TAB_TOP_START
    ...
    if (host.hasItemInput()) {
      ...
      tabs.addTabButton(
              new TabButton(page++)
                  .setBackground(
                      false,
                      ModularUITextures.VANILLA_TAB_TOP_MIDDLE.getSubArea(
                          0, 0, 1f, 0.5f),
    ...
    tabs.addTabButton(
            new TabButton(page++)
                ...
                .setBackground(
                    true,
                    ModularUITextures.VANILLA_TAB_TOP_MIDDLE.getSubArea(0, 0.5f,
                                                                        1f, 1f),
                    GT_UITextures.PICTURE_FLUID_OUT.withFixedSize(16, 16)
                        .withOffset(2, 4))
  ...
  protected void createMainTab(@Nonnull MultiChildWidget tab,
    ...
    tab.addChild(new DrawableWidget()
  ...
  protected void createItemInputTab(@Nonnull MultiChildWidget tab,
                                    @Nonnull Builder builder,
                                    @Nonnull UIBuildContext uiBuildContext) {
    tab.addChild(host.getItemLogic(InventoryType.Input, null)
                     .getGuiPart()
                     .setSize(18 * 4 + 9, 5 * 18)
                     .setPos(host.getWidth() / 2 - 2 * 18, 10));
  }

  protected void createItemOutputTab(@Nonnull MultiChildWidget tab,
                                     @Nonnull Builder builder,
                                     @Nonnull UIBuildContext uiBuildContext) {
    tab.addChild(host.getItemLogic(InventoryType.Output, null)
                     .getGuiPart()
                     .setSize(18 * 4 + 9, 5 * 18)
                     .setPos(host.getWidth() / 2 - 2 * 18, 10));
  }

  protected void createFluidInputTab(@Nonnull MultiChildWidget tab,
                                     @Nonnull Builder builder,
                                     @Nonnull UIBuildContext uiBuildContext) {
    tab.addChild(host.getFluidLogic(InventoryType.Input, null)
                     .getGuiPart()
                     .setSize(18 * 4 + 9, 5 * 18)
                     .setPos(host.getWidth() / 2 - 2 * 18, 10));
  }

  protected void createFluidOutputTab(@Nonnull MultiChildWidget tab,
                                      @Nonnull Builder builder,
                                      @Nonnull UIBuildContext uiBuildContext) {
    tab.addChild(host.getFluidLogic(InventoryType.Output, null)
                     .getGuiPart()
                     .setSize(18 * 4 + 9, 5 * 18)
                     .setPos(host.getWidth() / 2 - 2 * 18, 10));
  }

  protected void createPowerTab(@Nonnull MultiChildWidget tab,
                                @Nonnull Builder builder,
                                @Nonnull UIBuildContext uiBuildContext) {
    PowerLogic power = host.getPowerLogic();
    tab.addChild(TextWidget
                     .dynamicString(()
                                        -> power.getStoredEnergy() + "/" +
                                               power.getCapacity() + " EU")
                     .setPos(10, 30))
        .addChild(TextWidget
                      .dynamicString(()
                                         -> power.getVoltage() + " EU/t"
                                                + "(" + power.getMaxAmperage() +
                                                " A)")
                      .setPos(10, 60));
  }

  /**
   * Should return the logo you want to use that is pasted on each tab. Default
   * is the GT logo.
   */
  @Nonnull
  ...
  protected ButtonWidget createPowerSwitchButton(IWidgetBuilder<?> builder) {
    ...
    button
        ...
        .setBackground(() -> {
          if (host.isAllowedToWork()) {
            return new IDrawable[] {
                GT_UITextures.BUTTON_STANDARD_PRESSED,
        ...
        .attachSyncer(new FakeSyncWidget.BooleanSyncer(host::isAllowedToWork,
                                                       host::setAllowedToWork),
                      builder)
        .addTooltip(
            StatCollector.translateToLocal("GT5U.gui.button.power_switch"))
